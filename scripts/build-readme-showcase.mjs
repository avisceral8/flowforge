...
const outputPath = path.resolve(process.argv[2] || path.join(assetsRoot, 'flowforge-live-proof.gif'));
...
function commandPath(command) {
  const result = spawnSync('sh', ['-c', 'command -v "$1"', 'flowforge-showcase', command], {
    encoding: 'utf8',
...
<body>
  <iframe src="${esc(artifactUrl)}" title="${esc(scene.title)} generated FlowForge artifact"></iframe>
  <div class="edge"></div>
  <div class="topbar">
    <div class="brand">
      ...
      <span>FLOWFORGE</span><span class="live">LIVE PROOF</span>
    </div>
  ...
  <div class="fade"></div>
  <script>
    window.__flowforgeShowcaseReady=false;
    const frame=document.querySelector('iframe');
    frame.addEventListener('load',()=>setTimeout(()=>{window.__flowforgeShowcaseReady=true},260),{once:true});
  </script>
</body>
</html>`;
}
...
async function captureFrames(chromePath, tempRoot) {
  ...
  try {
    ...
    for (const [sceneIndex, scene] of scenes.entries()) {
      ...
      await loaded;
      await evaluate(cdp, sessionId, `new Promise((resolve,reject)=>{const end=Date.now()+12000;const poll=()=>window.__flowforgeShowcaseReady?resolve(true):Date.now()>end?reject(new Error('artifact load timeout')):setTimeout(poll,40);poll()})`, true);
...
async function main() {
  ...
  if (!chromePath) throw new Error('Chrome or Chromium is required. Set FLOWFORGE_CHROME to its executable path.');
  const ffmpeg = requireCommand('ffmpeg', 'Install it with your system package manager.');
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-readme-showcase-'));
  try {
