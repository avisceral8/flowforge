#!/usr/bin/env bash
# Build the distributable skill archive from the flowforge/ folder.
# Usage: scripts/build-zip.sh [output.zip]
set -euo pipefail

repo_root="$(cd "$(dirname "$0")/.." && pwd)"
out="${1:-$repo_root/flowforge.zip}"
if [[ "$out" != /* ]]; then
  out="$(pwd)/$out"
fi

# Runtime consumers support every Node version declared by flowforge/package.json,
...
if [[ "$node_major" != "$canonical_node_major" ]]; then
  echo "canonical flowforge.zip builds require Node $canonical_node_major (current: $node_version)" >&2
  exit 1
fi
...
node "$repo_root/scripts/stage-clean-skill.mjs" \
  --root "$repo_root" \
  --dest "$stage/flowforge" >/dev/null

node "$repo_root/scripts/write-deterministic-zip.mjs" "$stage/flowforge" "$out"
