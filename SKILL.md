---
name: flowforge
description: Turn a plain-language business process description into a polished, validated BPMN 2.0 process map as explorable standalone HTML with inline SVG, dark/light themes, optional trace motion, and PNG/JPEG/WebP/SVG/WebM export. Interview first (actors, trigger, happy path, gateways, exceptions, handoffs), confirm back, then render. Also inherits FlowForge's architecture, workflow, sequence, data-flow, and lifecycle/state diagrams as secondary technical types. Use when the user asks to map a business process, approval flow, order-to-cash, onboarding, incident handling, procurement, or an audit/compliance process.
license: MIT
metadata:
  version: "2.16"
  author: FlowForge (fork of tt-a1i/archify, MIT)
  based_on: tt-a1i/archify (MIT, v2.16.0-dev.0)
---
...
4. Validate after every candidate edit and immediately before handoff:
   ...
   ```bash
   node bin/flowforge.mjs validate <type> <candidate.json> --quality showcase --json
   ```
...
When ambiguous, run `node bin/flowforge.mjs guide "<scenario>" --json`. Scenario proof examples are structural references, not facts to copy.

## Mermaid input

Read Mermaid for topology and meaning, then author fresh FlowForge JSON; do not mechanically render Mermaid styling.
...
- Keep node labels short enough to read beside the semantic sigil: the renderer places each node's role icon in the top-left corner and the primary label below it, so a long label never overlaps the icon (locked by `test/bpmn-label-clearance.test.mjs`). Prefer a short label plus a `sublabel` over one long label.
...
- Brand identity is optional and explicit. Put a canonical built-in ID in `brand` when the node names that real product. If no preset matches and the user supplied the official HTTP(S) URL, first run `node bin/flowforge.mjs brands capture "<url>" --json`, then author the returned digest-pinned `brand` object. Render and validate never perform an unpinned capture. Otherwise omit `brand`. Never infer a brand from a vague role such as "database", and never let a badge replace the semantic `type`, label, or relationship facts.
...
```bash
node bin/flowforge.mjs visual-check <output.html> --json
```
...
```bash
node bin/flowforge.mjs preview <type> <input>.json <output>.html --quality showcase
```
...
node bin/flowforge.mjs demo <output-directory>
```
