...
Thank you for helping FlowForge make engineering diagrams more trustworthy and useful. FlowForge is Agent-first: people describe the system they want to explain, while the Skill, typed JSON contract, renderers, validators, and delivery receipts make the result reproducible.
...
FlowForge's public behavior is larger than a renderer function. Treat these as contracts:
...
The renderer package lives in `flowforge/` and supports Node.js 18 and later. CI covers Node.js 18, 20, 22, and 24.
...
Test public behavior through a supported seam whenever possible: `flowforge render`, `validate`, `deliver`, `visual-check`, or the final SVG/HTML. Private helper tests are useful for edge cases, but they do not replace a CLI or artifact-level regression.
...
```bash
cd flowforge
FLOWFORGE_CHROME="/path/to/chrome" node --test test/desktop-reader-browser.test.mjs
```
...
Preserve non-zero exit behavior and machine-readable receipts. A successful `validate` does not prove atomic delivery, and a successful `deliver` does not prove perceptual quality. Follow [the delivery contract](flowforge/references/delivery-contract.md) and test the failure stage you changed.
...
The runtime follows the Node range in `flowforge/package.json`, but canonical
`flowforge.zip` container bytes are built only with Node 22. The builder rejects
other Node majors so a different bundled zlib cannot publish a second byte
representation of the same package contents.

Bundled example or viewer changes normally require the Gallery rebuild. Skill runtime, schema, renderer, or published `SKILL.md` changes require checking `flowforge.zip` freshness and committing a rebuilt archive when the checked-in package contents differ.
...
1. The exact FlowForge version or commit, installation method, command, and environment.
...
Showcase cases should be reproducible proof, not promotional screenshots. Submit the original prompt, agent/client, exact model, FlowForge version, redacted typed JSON, artifact, validation receipt, and truthful visual-review status through `.github/ISSUE_TEMPLATE/showcase.yml`.
