...
Community DeepSeek Harness integration for [FlowForge](https://github.com/tt-a1i/archify). This is **not** an official DeepSeek product and does not imply DeepSeek endorsement.
...
The package is a Skill-only bundle: it inserts one filesystem Skill provider named `flowforge-plugin` and exposes the FlowForge 2.14 snapshot released with this package. It does not register native render/validate/deliver tools, a custom Web client, Produced Files chips, telemetry, network access, credentials handling, background services, or `prepare` / `install` / `postinstall` hooks.

Release maintenance is immutable: rebuilding `0.1.0` reads its payload from the `flowforge-dsh-v0.1.0` tag. Later FlowForge changes, including the embedded update notifier, are intentionally excluded until a separately authorized DSH release receives a new version.
...
Ask DSH to load FlowForge by name:

```text
Use the flowforge skill to map this repository's runtime architecture.
...
FlowForge then runs through DSH's ordinary Skill, shell, and filesystem paths. Generated JSON and HTML are normal workspace files.
