...
const PROFILE = 'flowforge-dsh-acceptance';
...
const scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-dsh-acceptance-'));
const tarball = path.join(scratch, 'tt-a1i-flowforge-dsh-0.1.0.tgz');
...
if (!packedFiles.includes('skills/flowforge/SKILL.md')) {
  fail('tarball-inspect', 'packed tarball is missing the clean FlowForge Skill');
}
...
const dshEnv = {
  ...
  FLOWFORGE_DSH_PROBE_OUT: probeOut,
...
const flowforgeLayer = composed.layers.find((layer) => layer.name === PACKAGE_NAME);
const originalFilesystem = composed.rows.find((row) => row.id === 'skill-filesystem');
const flowforgeProvider = composed.rows.find((row) => row.id === 'flowforge-skill-filesystem');
const extraProviders = composed.rows.filter((row) => row.config.providerName === 'flowforge-plugin');
if (!dump.stdout.includes(`# == ${PACKAGE_NAME}`) || !flowforgeLayer) {
  fail('compose', 'composed dump does not include the FlowForge bundle layer', { layers: composed.layers.map((layer) => layer.name) });
}
if (!originalFilesystem || originalFilesystem.config.providerName === 'flowforge-plugin') {
  fail('compose', 'original DSH skill-filesystem row was replaced', { originalFilesystem });
}
if (!flowforgeProvider || extraProviders.length !== 1 || flowforgeLayer.rows.length !== 1) {
  fail('compose', 'composed config did not insert exactly one FlowForge Skill provider', {
    extra: extraProviders.map((row) => row.id),
    layerRows: flowforgeLayer.rows.map((row) => row.id),
  });
}
if (flowforgeProvider.config.includeDefaultRoots !== false || flowforgeProvider.config.providerName !== 'flowforge-plugin') {
  fail('compose', 'FlowForge provider config is not isolated', { flowforgeProvider });
}
pass('compose', {
  extraIds: flowforgeLayer.rows.map((row) => row.id),
  providerName: 'flowforge-plugin',
});
...
const flowforgeHits = (probeReceipt.skills || []).filter((skill) => skill.name === 'flowforge');
if (flowforgeHits.length !== 1 || flowforgeHits[0].provider !== 'flowforge-plugin') {
  fail('skill-discovery', 'public Skill registry did not discover flowforge only from flowforge-plugin', {
    probeReceipt,
    stdout: probeStdout,
    stderr: probeStderr,
  });
}
pass('skill-discovery', { provider: 'flowforge-plugin' });

if (!probeReceipt.definition?.contentLength || probeReceipt.definition.provider !== 'flowforge-plugin') {
...
const installedPackage = path.join(profileDir, 'node_modules', '@tt-a1i', 'flowforge-dsh');
let resourceReal;
let packageReal;
try {
...
if (!resourceInsidePackage || !resourceReal.includes(`${path.sep}skills${path.sep}flowforge`)) {
...
const skillRoot = fs.existsSync(path.join(resourceReal, 'SKILL.md'))
  ? resourceReal
  : path.join(resourceReal, 'flowforge');
...
const leftover = parseDump(baseBootDump.stdout).rows.filter((row) => (
  row.id === 'flowforge-skill-filesystem' || row.config.providerName === 'flowforge-plugin'
));
if (leftover.length > 0) {
  fail('base-profile', 'uninstalled profile still contains the FlowForge provider', { leftover });
}
pass('base-profile', { bundles: removedManifest.dsh?.profile?.bundles || [] });

const zipBlob = run('git', ['hash-object', 'flowforge.zip'], { cwd: repoRoot });
const pkgBlob = run('git', ['hash-object', 'flowforge/package.json'], { cwd: repoRoot });
const skipFreshZipRebuild = process.platform === 'win32';
const committedZip = path.join(repoRoot, 'flowforge.zip');
...
if (skipFreshZipRebuild) {
  ...
  const currentSmoke = run(process.execPath, [
    ...
    path.join(checkedDir, 'flowforge'),
  ...
  const unzipDiff = run('diff', ['-r', path.join(freshDir, 'flowforge'), path.join(checkedDir, 'flowforge')]);
...
pass('zero-regression', {
  flowforgeZipBlob: zipBlob.stdout.trim(),
  flowforgePackageBlob: pkgBlob.stdout.trim(),
