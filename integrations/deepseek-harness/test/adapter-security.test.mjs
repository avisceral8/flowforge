...
test('host-loaded adapter code does not open a second execution, network, credential, or telemetry surface', () => {
  ...
  assert.doesNotMatch(source, /flowforge_render|flowforge_deliver|tools\.register/);
});

test('package resolution failures fail loud instead of returning a guessed Skill root', async () => {
  const { resolveFlowForgeSkillRoot } = await import('../lib/index.js');
  assert.throws(() => resolveFlowForgeSkillRoot('file:///tmp/flowforge-dsh-missing-profile/'));
  assert.throws(() => resolveFlowForgeSkillRoot(''));
});
