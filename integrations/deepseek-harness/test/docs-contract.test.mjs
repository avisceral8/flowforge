...
test('README.md stays the single English doc after the DSH docs', () => {
  assert.match(read('README.md'), /FlowForge/);
});
...
test('DSH 0.1.0 documentation keeps its released Skill snapshot immutable', () => {
  ...
  assert.match(integration, /FlowForge 2\.14 snapshot/);
  assert.match(integration, /flowforge-dsh-v0\.1\.0/);
  assert.match(integration, /update notifier[\s\S]*intentionally excluded/);
});

test('English README covers install, invoke, uninstall, community wording, and Produced Files', () => {
  const english = [read('README.md'), read('integrations/deepseek-harness/README.md')].join('\n');

  for (const source of [english, read('README.md')]) {
    assert.match(source, /@tt-a1i\/flowforge-dsh@0\.1\.0/);
    assert.match(source, /@deepseek-ai\/dsh@0\.1\.0-rc\.6/);
    assert.match(source, /\^22\.19\.0 \|\| >=24\.0\.0/);
    assert.match(source, /dsh plugin --profile web add @tt-a1i\/flowforge-dsh@0\.1\.0/);
    assert.match(source, /dsh plugin --profile web remove @tt-a1i\/flowforge-dsh/);
    assert.match(source, /Use the flowforge skill to map this repository's runtime architecture/);
    assert.doesNotMatch(source, /dsh plugin[^\n]*github:tt-a1i\/flowforge/);
...
test('Skills CLI, Cursor, Codex, Claude Code, OpenCode, and Raven remain the default main path', () => {
  ...
  assert.match(english, /^```bash\nnpx skills add tt-a1i\/flowforge -g\n```$/m);
