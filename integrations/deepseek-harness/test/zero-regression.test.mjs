...
test('the repository still has exactly one checked-in FlowForge SKILL.md and no generated DSH payload', () => {
  const skillFiles = git(['ls-files', '*SKILL.md']).split('\n').filter(Boolean);
  assert.deepEqual(skillFiles, ['flowforge/SKILL.md']);
...
test('FlowForge core does not import, detect, or branch on DeepSeek Harness', () => {
  const grep = spawnSync('git', [
    'grep',
    '-n',
    '-E',
    'deepseek-harness|@deepseek-ai/dsh|DSH_HOME|DSH_AGENTS_HOME|flowforge-dsh',
    '--',
    'flowforge',
...
test('full-depth Skills CLI discovery still finds only one skill named flowforge', () => {
  ...
  const names = [...result.stdout.matchAll(/^\s*[-*]\s+(\S+)/gm)].map((match) => match[1])
    .filter((name) => name === 'flowforge' || /flowforge/i.test(name));
  const unique = new Set(
    [...result.stdout.matchAll(/\bflowforge\b/gi)].map((match) => match[0].toLowerCase()),
  );
  assert.ok(result.stdout.includes('flowforge'), result.stdout);
