...
const cliPath = path.join(here, 'flowforge.mjs');
const loopbackHost = '127.0.0.1';
const defaultDebounceMs = 400;
const defaultPollMs = 800;
const defaultStopGraceMs = 3000;
const defaultStopKillMs = 750;
const diagramTypes = new Set(['bpmn', 'architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'journey', 'infoflow', 'stakeholder-map', 'capability', 'okr-tree', 'north-star', 'growth-loop', 'launch-plan', 'feedback-pipeline']);
...
<head>
  ...
  <title>FlowForge Live Preview</title>
  <style>
...
<body data-state="checking" data-has-artifact="false">
  <header>
    <span class="brand">FlowForge Preview</span>
  ...
  <main>
    ...
    <iframe id="artifact" title="Verified FlowForge diagram"></iframe>
  </main>
  <script>
    (function () {
      'use strict';
...
export async function startPreview(options) {
  ...
  const stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.flowforge-preview-'));
    ...
    state.failure = {
      ...
      message: redactDiagnostic(
        ...
        [
          ...
          [path.resolve(here, '..'), '<flowforge-skill>'],
