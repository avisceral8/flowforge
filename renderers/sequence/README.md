...
Render `diagram_type: "sequence"` JSON files into the standard FlowForge HTML
template.

```bash
node flowforge/renderers/sequence/render-sequence.mjs input.sequence.json output.html
```

The renderer validates input against `flowforge/schemas/sequence.schema.json`
...
`flowforge/examples/cache-miss-request.sequence.json`.

The schema lives at:

```text
flowforge/schemas/sequence.schema.json
```
