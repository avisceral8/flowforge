...
const RESOLVED_MARK = Symbol('flowforge.brandMark');
const MAX_HTML_BYTES = 256 * 1024;
const MAX_IMAGE_BYTES = 1024 * 1024;
const MAX_CAPTURE_CONCURRENCY = 3;
const DEFAULT_CAPTURE_TIMEOUT_MS = 8000;
const USER_AGENT = 'FlowForge/2.15 brand-preview';
...
function validateUrlShape(url, allowPrivate = process.env.FLOWFORGE_BRAND_ALLOW_PRIVATE === '1') {
...
async function resolveRequestTarget(url, deadline) {
  const allowPrivate = process.env.FLOWFORGE_BRAND_ALLOW_PRIVATE === '1';
...
function captureTimeoutMilliseconds() {
  const configured = Number(process.env.FLOWFORGE_BRAND_CAPTURE_TIMEOUT_MS);
...
export async function prepareDiagramBrandMarks(diagramType, diagram) {
  ...
  await mapConcurrent(nodes, MAX_CAPTURE_CONCURRENCY, async (node, index) => {
    ...
    if (url) {
      unknown.push(`/${collection}/${index}/brand ${JSON.stringify(node.brand)} is an unpinned URL; capture it first with \`flowforge brands capture ${url.href} --json\``);
      return;
    }
  ...
  if (unknown.length) {
    throwDiagnosticError(`Brand mark validation failed:\n- ${unknown.join('\n- ')}`, unknown.map((message) => ({
      ...
      supportedFixes: message.includes('is an unpinned URL')
        ? ['run `flowforge brands capture <url> --json` and author the returned digest-pinned brand object']
        : ['choose an ID from `flowforge brands`', 'run `flowforge brands capture <url> --json` for an unknown official site'],
    })));
  }
}
