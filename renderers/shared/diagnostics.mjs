...
const DIAGNOSTIC_MODE = process.env.FLOWFORGE_DIAGNOSTIC_FORMAT === 'json';
const recorded = [];
const recordedMessages = new Set();
const boundaryKey = Symbol.for('flowforge.renderer-diagnostic-boundary');
...
function normalizedDiagnostic(diagnostic) {
  const message = String(diagnostic?.message || 'FlowForge could not classify this failure.').trim();
  return {
...
export function throwDiagnosticError(message, diagnostics) {
  ...
  error.flowforgeDiagnostics = (diagnostics || []).map(normalizedDiagnostic);
  throw error;
}
...
function rendererFailure(error) {
  const attached = Array.isArray(error?.flowforgeDiagnostics)
    ? error.flowforgeDiagnostics.map(normalizedDiagnostic)
    : [];
