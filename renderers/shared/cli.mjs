...
export function loadDiagram({ rendererDir, diagramType, defaultExample, argv = process.argv }) {
  ...
  const sourceEvidence = verifyRepositoryEvidence(diagramType, diagram, process.env.FLOWFORGE_REPO_ROOT);
...
const START_TYPES = new Set(['bpmn', 'architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'journey', 'infoflow', 'stakeholder-map', 'capability', 'okr-tree', 'north-star', 'growth-loop', 'launch-plan', 'feedback-pipeline']);
...
const RELATIONSHIP_COLLECTIONS = {
  ...
  journey: 'edges',
  infoflow: 'edges',
  'stakeholder-map': 'edges',
  capability: 'edges',
  'okr-tree': 'edges',
  'north-star': 'edges',
  'growth-loop': 'edges',
  'launch-plan': 'edges',
  'feedback-pipeline': 'edges',
};
...
export function svgRootAttrs(meta) {
  ...
  const engineeringProfile = meta.engineering_profile
    ...
    : '';
  const requestedProfile = process.env.FLOWFORGE_QUALITY_PROFILE || meta.quality_profile;
  ...
  return `role="img" lang="${esc(resolveLocale(meta.locale))}" aria-labelledby="flowforge-diagram-title flowforge-diagram-description"${animation}${preset}${engineeringProfile} data-quality-profile="${esc(qualityProfile)}"${advisory}`;
}

// Keep the accessible name inside the SVG so it survives standalone SVG
// export and embedding. The fixed IDs are deterministic because an FlowForge
...
export function svgAccessibleText(meta, kind) {
  ...
  return `        <title id="flowforge-diagram-title">${esc(meta.title)}</title>\n        <desc id="flowforge-diagram-description">${esc(description)}</desc>`;
}
...
// canonical SVG is embedded inline outside the full FlowForge viewer.
