...
const MESSAGE_PAIRS = {
  ...
  'diagram.description.architecture': ['An architecture diagram generated by FlowForge.', '由 FlowForge 生成的架构图。'],
  'diagram.description.workflow': ['A workflow diagram generated by FlowForge.', '由 FlowForge 生成的工作流图。'],
  'diagram.description.sequence': ['A sequence diagram generated by FlowForge.', '由 FlowForge 生成的时序图。'],
  'diagram.description.dataflow': ['A data-flow diagram generated by FlowForge.', '由 FlowForge 生成的数据流图。'],
  'diagram.description.lifecycle': ['A lifecycle diagram generated by FlowForge.', '由 FlowForge 生成的生命周期图。'],
  ...
  'viewer.preset.badge.editorialPlate': ['FLOWFORGE / PLATE 04', 'FLOWFORGE / 图版 04'],
  ...
  'viewer.export.card.routeBadge': ['FLOWFORGE · ROUTE · {hops}', 'FLOWFORGE · 路径 · {hops}'],
  'viewer.export.card.reachBadge': ['FLOWFORGE · {direction} REACH', 'FLOWFORGE · {direction}可达范围'],
  'viewer.export.card.defaultBadge': ['FLOWFORGE · {preset} · {theme}', 'FLOWFORGE · {preset} · {theme}'],
...
for (const [key, messages] of Object.entries(MESSAGE_PAIRS)) {
  if (messages.length !== SUPPORTED_LOCALES.length || messages.some((message) => typeof message !== 'string')) {
    throw new Error(`Incomplete FlowForge i18n tuple ${JSON.stringify(key)}`);
  }
}
...
export function translateMessage(locale, key, values = {}) {
  ...
  if (!Object.hasOwn(CATALOGS[resolved], key)) {
    throw new Error(`Missing FlowForge i18n message ${JSON.stringify(key)} for ${resolved}`);
  }
