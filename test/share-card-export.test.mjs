...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-share-card-'));
...
test('Share Card uses contain-only canonical geometry with fixed safe areas', () => {
  ...
  assert.match(html, /FLOWFORGE ·/);
  assert.doesNotMatch(svgBlock(html), /share-card|Share Card|FLOWFORGE ·/);
});
