...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-guided-views-'));
...
for (const [mode, config] of Object.entries(CASES)) {
  test(`${mode}: guided views preserve base SVG geometry`, () => {
    ...
    assert.match(guided.html, /FlowForge\.guidedViews = \(function \(\)/);
    ...
    assert.match(guided.html, /--flowforge-scroll-x/);
