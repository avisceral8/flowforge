...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-story-follow-camera-'));
...
test('Story Follow frames the exact previous, current, and next authored stops through the shared camera', () => {
  ...
  assert.match(template, /FlowForge\.view\.reveal\(ids, \{/);
...
test('adaptive dwell, Still, reduced motion, hidden pages, and print keep camera motion bounded', () => {
  ...
  assert.match(template, /FlowForge\.motionGovernor && FlowForge\.motionGovernor\.capable\) return !FlowForge\.motionGovernor\.isPaused\(\)/);
...
test('pause, settle, overview, and manual camera takeover cancel Story Follow state', () => {
  ...
  assert.match(template, /function interruptCamera\(reason\)[\s\S]*?FlowForge\.guidedViews\.pause\(\)/);
