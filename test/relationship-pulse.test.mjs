...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-relationship-pulse-'));
...
test('semantic tokens use five distinct inline SVG cues on one finite timing owner', () => {
  ...
  assert.match(html, /animation: flowforge-relationship-token-life 1\.2s linear 1 both/);
  assert.match(html, /semantic-flow-token-halo/);
  assert.match(html, /FlowForge\.flowTokens = \{/);
...
test('pulse is finite, event-owned, preset-aware, touch-safe, and motion-safe', () => {
  ...
  assert.match(html, /animation: flowforge-relationship-pulse 1\.2s linear 1 both/);
  assert.match(html, /@keyframes flowforge-relationship-token-life/);
  ...
  assert.match(html, /FlowForge\.motionGovernor && FlowForge\.motionGovernor\.isPaused\(\)/);
