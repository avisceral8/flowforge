...
test('preview contract: the skill keeps live preview explicit, desktop-only, and last-good', () => {
  assert.match(delivery, /flowforge\.mjs preview <type> <input>\.json <output>\.html/);
...
test('preview contract: the README documents the same optional command without changing the hero', () => {
  assert.match(readme, /bin\/flowforge\.mjs preview workflow/);
  assert.match(readme, /--no-open/);
  assert.match(readme, /127\.0\.0\.1/);
  assert.match(readme, /Ctrl-C/);
  assert.match(readme, /docs\/assets\/flowforge-readme-hero\.png/);
});
