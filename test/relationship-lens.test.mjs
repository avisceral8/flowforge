...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-relationship-lens-'));
...
test('relationship lens groups incoming, outgoing, and self-loop paths and follows neighbors', () => {
  ...
  assert.match(html, /FlowForge\.view\.reveal\(\[id\], \{ includeNeighbors: true, reason: 'relationship' \}\)/);
