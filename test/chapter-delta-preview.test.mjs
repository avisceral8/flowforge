...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-chapter-delta-preview-'));
...
test('pointer and keyboard inspect while touch and native activation still commit directly', () => {
  ...
  assert.doesNotMatch(previewRuntime, /FlowForge\.view\.|updateUrl\(|FlowForge\.focus\.|renderStoryTrail\(/);
});
...
test('latest intent, stronger owners, playback, and lifecycle cleanup remain bounded', () => {
  ...
  assert.match(html, /FlowForge\.motionGovernor\.claim\('chapter-preview'/);
  assert.match(html, /FlowForge\.motionGovernor\.release\(token\)/);
