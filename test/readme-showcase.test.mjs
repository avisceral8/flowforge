...
const assetPath = path.join(repoRoot, 'docs', 'assets', 'flowforge-live-proof.gif');
const receiptPath = path.join(repoRoot, 'docs', 'assets', 'flowforge-live-proof.json');
...
test('README motion proof is compact, looping, and backed by current gallery artifacts', () => {
  ...
  assert.equal(receipt.output, 'docs/assets/flowforge-live-proof.gif');
...
test('all README languages keep the product hero and retain the verified animated proof', () => {
  for (const filename of ['README.md']) {
    ...
    const heroIndex = readme.indexOf('docs/assets/flowforge-readme-hero.png');
    const titleIndex = readme.indexOf('# FlowForge');
    const proofIndex = readme.indexOf('docs/assets/flowforge-live-proof.gif');
    const demosIndex = Math.max(readme.indexOf('## See FlowForge in action'), readme.indexOf('## 看看 FlowForge 能做什么'));
    ...
    assert.match(readme, /docs\/assets\/flowforge-live-proof\.gif/);
    assert.match(readme, /https:\/\/tt-a1i\.github\.io\/flowforge\/gallery\.html/);
  }
});
...
test('README stays scannable without deleting the visual proof set', () => {
  const commonAssets = [
    'flowforge-readme-hero.png',
    'flowforge-live-proof.gif',
    'flowforge-demo-story.png',
    'flowforge-demo-route.png',
    'flowforge-demo-lens.png',
    'mco-runtime-share-card.png',
    'flowforge-dark.png',
    'flowforge-light.png',
    'flowforge-menu.png',
    'flowforge-workflow.png',
    'flowforge-sequence.png',
    'flowforge-dataflow.png',
    'flowforge-lifecycle.png',
  ];
...
test('Star History publishing advances the data branch without a force push', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-star-history-'));
