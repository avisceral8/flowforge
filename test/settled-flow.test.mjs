...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-settled-flow-'));
...
test('all five renderers inherit one finite running-to-settled ambient contract', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /animation: flowforge-edge-flow [^;]+ 1;/, mode);
    assert.match(html, /animation: flowforge-node-pulse [^;]+ 1;/, mode);
    assert.doesNotMatch(html, /animation: flowforge-edge-flow [^;]+ infinite/, mode);
    assert.doesNotMatch(html, /animation: flowforge-node-pulse [^;]+ infinite/, mode);
  }
});
...
test('settled flow restores authored security and async dash semantics', () => {
  ...
  assert.match(template, /@keyframes flowforge-edge-flow\s*\{[\s\S]*?stroke-dasharray:\s*10 8/);
