...
canonicalZipTest('package smoke rejects every dependency metadata field in a built package', () => {
  ...
  try {
    ...
    const builtPackage = path.join(extracted);
...
canonicalZipTest('built archives contain the embedded notifier runtime', () => {
  ...
  try {
    ...
    assert.ok(entries.has('skill-release.json'));
    assert.ok(entries.has('scripts/check-update.mjs'));
    assert.ok(entries.has('scripts/update-contract.mjs'));
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
});
...
canonicalZipTest('archive build excludes untracked files and external symlinks from the live working tree', () => {
  ...
  const untracked = path.join(repoRoot, `${marker}.txt`);
  ...
  const externalLink = path.join(repoRoot, `${marker}.link`);
