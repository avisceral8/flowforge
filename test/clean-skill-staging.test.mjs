...
function repositoryFixture() {
  ...
  write(root, 'skill-release.json', '{}\n');
  write(root, 'scripts/check-update.mjs', 'export {};\n');
  write(root, 'scripts/update-contract.mjs', 'export {};\n');
  write(root, 'renderers/shared/generated-validators.mjs', 'export {};\n');
  write(root, 'test/repository-only.test.mjs', 'throw new Error();\n');
  git(root, ['init']);
  return root;
}
...
test('clean staging preserves index modes and strips repository-only package metadata', () => {
  ...
  try {
    write(root, 'bin/executable.mjs', '#!/usr/bin/env node\n', 0o755);
    write(root, 'runtime/test/required.dat', 'runtime fixture\n');
    git(root, ['add', '.']);
...
test('clean staging rejects a symlink in a tracked file ancestor before copying bytes', (t) => {
  ...
  try {
    ...
    write(root, 'runtime/payload.txt', 'tracked fixture\n');
    git(root, ['add', '.']);
    ...
    assert.throws(
      () => stageCleanSkill({ repoRoot: root, destination }),
      /refusing to package path through symlink: runtime/,
    );
...
test('clean staging rejects tracked symlinks before reading through them', (t) => {
  ...
  try {
    ...
    assert.throws(
      () => stageCleanSkill({ repoRoot: root, destination }),
      /refusing to package tracked symlink: linked.txt/,
    );
    assert.equal(fs.existsSync(destination), false);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
...
test('clean staging snapshots unstaged tracked bytes before a source ancestor can be swapped', (t) => {
  ...
  let swapped = false;
  try {
    const payload = write(root, 'runtime/payload.txt', 'indexed fixture\n');
    write(root, 'outside-runtime/payload.txt', 'external secret\n');
    git(root, ['add', '.']);
...
test('clean staging rejects a source ancestor swapped during preflight traversal', (t) => {
  ...
  try {
    ...
    write(root, 'outside-runtime/payload.txt', 'external secret\n');
    git(root, ['add', '.']);
    const canonicalRuntime = path.join(fs.realpathSync(root), 'runtime');
    ...
    assert.throws(
      () => stageCleanSkill({ repoRoot: root, destination }),
      /(?:tracked package path changed before it could be read: runtime\/payload\.txt|tracked package input is missing or unreadable: runtime\/payload\.txt)/,
    );
