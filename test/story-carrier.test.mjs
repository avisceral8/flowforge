...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-story-carrier-'));
...
test('all five renderers inherit one viewer-only Semantic Story Carrier', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /FlowForge\.flowTokens = \{/, mode);
    assert.match(html, /className: 'story-flow-token'/, mode);
    assert.match(html, /data-story-carrier-token/, mode);
    assert.match(html, /animation: flowforge-relationship-token-life 0\.78s linear 1 both/, mode);
...
test('Story reuses the exact semantic token vocabulary on its existing finite edge pulse', () => {
  ...
  assert.match(template, /FlowForge\.flowTokens\.create\(edge, shapes\[0\], \{/);
...
test('Story Carrier cleanup and export remain owned by Story Trail', () => {
  ...
  assert.equal((template.match(/storyPulseOwnerToken = FlowForge\.motionGovernor\.claim\('story'/g) || []).length, 1);
