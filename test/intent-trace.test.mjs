...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-intent-trace-'));
...
test('all typed renderers inherit one geometry-neutral Intent Trace', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /FlowForge\.intentTrace = \(function \(\)/, mode);
...
test('Intent Trace keeps incoming and outgoing motion on authored source-to-target geometry', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /@keyframes flowforge-intent-trace-flow[\s\S]+stroke-dashoffset: -1/, mode);
  }
});
...
test('Intent Trace separates hover, keyboard, touch, and committed focus', () => {
  ...
  assert.match(html, /FlowForge\.focus\.active\(\)/);
  assert.match(html, /FlowForge\.intentTrace\.clear\(\{ announce: false \}\)/);
  assert.match(html, /e\.key === 'Escape' && FlowForge\.intentTrace\.active\(\)/);
});
...
test('Intent Trace normalizes motion, respects reduced motion, and exports cleanly', () => {
  ...
  assert.match(html, /@keyframes flowforge-intent-trace-flow/);
  assert.match(html, /animation: flowforge-intent-trace-flow 1\.15s linear 1 both/);
