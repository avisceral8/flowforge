...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-story-shelf-'));
...
test('active stories expand through existing state without storage or a second interaction owner', () => {
  ...
  assert.doesNotMatch(template, /storyShelf(?:Open|Expanded|Storage)|flowforge-story-shelf|localStorage[^\n]*shelf/i);
});
...
test('Story Shelf remains viewer-only, embed-safe, print-safe, and motion-neutral', () => {
  ...
  assert.match(template, /\.toolbar, \.diagram-nav, \.focus-chip, \.guided-views, \.flowforge-toast, \.no-print \{ display: none !important; \}/);
  assert.doesNotMatch(canonicalSvg(render('workflow')), /Story Shelf|guided-view|data-active-view/);
  assert.doesNotMatch(template, /@keyframes\s+flowforge-story-shelf|animation:[^;]*story-shelf/i);
});
