...
const pinnedMcoRoot = process.env.FLOWFORGE_MCO_REPO_ROOT
  ? path.resolve(process.env.FLOWFORGE_MCO_REPO_ROOT)
  : automaticMcoRoot();

test('MCO showcase preserves checked-in connection-label geometry', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-mco-showcase-layout-'));
  try {
...
test('checked-in MCO artifacts are byte-reproducible from the pinned repository revision', {
  skip: pinnedMcoRoot
    ? false
    : `Set FLOWFORGE_MCO_REPO_ROOT to a matching ${pinnedRepository.url} clone containing revision ${pinnedRepository.revision.slice(0, 7)}.`,
}, () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-mco-byte-reproduction-'));
  try {
...
test('MCO public proof is source-backed, valid, and linked from every README', () => {
  ...
  assert.match(checkedInHtml, /FlowForge\.sourceEvidence\.installBeacons\(\)/);
  execFileSync(process.execPath, [cli, 'check', artifactPath], { encoding: 'utf8' });

  const noRootTmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-mco-proof-no-root-'));
  try {
