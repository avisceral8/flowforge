...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-viewer-chrome-layout-'));
const chromePath = process.env.FLOWFORGE_CHROME ? findChrome() : null;
...
function render(mode, example) {
  ...
  execFileSync(process.execPath, [
    path.join(skillRoot, 'bin', 'flowforge.mjs'),
    'render',
    mode,
      ...
      return new Promise(function (resolve, reject) {
        ...
        function sample() {
          ...
          var current = [
            ...
            container ? getComputedStyle(container).getPropertyValue('--flowforge-nav-reserve') : ''
          ].join('|');
...
async function finalGeometry(browser, sessionId) {
  return evaluate(browser, sessionId, `(function () {
    ...
    var chromeReceipt = window.FlowForge && FlowForge.viewerChromeLayout
    ...
    var semanticDockIntersectionArea = stageDockIntersectionArea > 0 && navRect && svg
      ...
      : 0;
    return {
      reserve: parseFloat(getComputedStyle(container).getPropertyValue('--flowforge-nav-reserve')) || 0,
...
test('the public CLI gives all typed renderers one final Viewer contract', () => {
  ...
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.doesNotMatch(canonicalSvg(html), /nav-safe-rail|flowforge-nav-reserve|viewerChromeLayout/, mode);
    execFileSync(process.execPath, [path.join(skillRoot, 'bin', 'flowforge.mjs'), 'check', output]);
  }
});
...
test('Dock Safe Rail keeps typed renderers clear across themes, Presentation, and low-height desktops', {
  skip: chromePath ? false : 'Set FLOWFORGE_CHROME to run the real browser regression.',
}, async () => {
  ...
  try {
    ...
    for (const viewport of [
      ...
      await evaluate(browser, sessionId, `(function () {
        ...
        FlowForge.focus.set('lb', { toggle: false });
        FlowForge.radar.open();
        window.dispatchEvent(new Event('resize'));
      })()`);
      await waitForLayout(browser, sessionId);
      const receipt = await finalGeometry(browser, sessionId);
