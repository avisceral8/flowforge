...
const cursorCommand = 'npx -y skills add tt-a1i/archify --skill flowforge --agent cursor --global --copy --yes';

test('Cursor onboarding stays explicit, English-only, and backed by the same Skill', () => {
  ...
  for (const surface of [english, start, landing]) {
    ...
    assert.doesNotMatch(surface, /~\/\.cursor\/skills\/flowforge/);
...
test('the zero-dependency archive works from the canonical Cursor-visible agent path', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-cursor-package-'));
  const agentSkills = path.join(tmp, '.agents', 'skills');
  try {
    fs.mkdirSync(agentSkills, { recursive: true });
    execFileSync('unzip', ['-q', path.join(repoRoot, 'flowforge.zip'), '-d', agentSkills]);
    const installed = path.join(agentSkills, 'flowforge');
    const cli = path.join(installed, 'bin', 'flowforge.mjs');
    const doctor = execFileSync(process.execPath, [cli, 'doctor'], { encoding: 'utf8' });
    assert.match(doctor, /FlowForge is ready\./);
