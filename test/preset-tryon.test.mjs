...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-preset-tryon-'));
...
function presetRuntime(html) {
  return html.match(/FlowForge\.preset = \(function \(\) \{[\s\S]*?\n    \}\)\(\);/)?.[0] || '';
}
...
test('all five renderers expose one reader-controlled visual style picker', () => {
  for (const mode of Object.keys(CASES)) {
    ...
    assert.match(html, /FlowForge\.preset = \(function \(\)/, mode);
...
test('omitted visual preset opens as Classic and theme switching cannot change it', () => {
  const html = render('architecture');
  const themeRuntime = html.match(/FlowForge\.theme = \(function \(\) \{[\s\S]*?\n    \}\)\(\);/)?.[0] || '';
  ...
  assert.doesNotMatch(themeRuntime, /data-preset|FlowForge\.preset/);
});
