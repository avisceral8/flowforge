...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-diagram-guide-'));
...
test('all typed renderers inherit one viewer-only Diagram Guide', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /FlowForge\.guide = \(function \(\)/, mode);
    assert.match(html, /Diagram Guide — a factual command deck over existing interactions/, mode);
    assert.doesNotMatch(canonicalSvg(html), /diagram-guide|FlowForge\.guide|Explore this system/, mode);
  }
});
...
test('Diagram Guide reports compiled semantic facts and honest story availability', () => {
  ...
  assert.match(html, /return FlowForge\.guidedViews && Number\(FlowForge\.guidedViews\.count\) \|\| 0/);
...
test('Diagram Guide delegates its task rows to existing production interactions', () => {
  ...
  assert.match(html, /if \(action === 'find'\) return FlowForge\.finder\.open\(\)/);
  assert.match(html, /if \(action === 'route'\) return FlowForge\.routeProbe\.begin\(\{ focusNode: true \}\)/);
  assert.match(html, /if \(action === 'map'\) return FlowForge\.radar\.open\(\)/);
  assert.match(html, /if \(action === 'story'\) return FlowForge\.guidedViews\.play\(\)/);
  assert.match(html, /if \(action === 'present'\) return FlowForge\.presentation\.enter\(\)/);
  assert.match(html, /if \(action === 'export'\) return FlowForge\.exportMenu\.open\(\)/);
  assert.match(html, /if \(action === 'theme'\) return FlowForge\.theme\.toggle\(\)/);
  assert.match(html, /if \(action === 'reset'\) return FlowForge\.view\.reset\(\)/);
  assert.match(html, /FlowForge\.guidedViews\.pause\(\)/);
  assert.match(html, /FlowForge\.finder\.close\(\{ restoreFocus: false \}\)/);
  assert.match(html, /FlowForge\.radar\.close\(\{ restoreFocus: false \}\)/);
...
test('Diagram Guide is keyboard-first, mobile-contained, motion-safe, and embed-clean', () => {
  ...
  assert.match(html, /FlowForge\.guide\.toggle\(\)/);
