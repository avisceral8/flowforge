...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-chapter-rail-'));
...
test('chapter rail has positional, touch, mobile, motion, embed, and print boundaries', () => {
  ...
  assert.match(html, /\.toolbar, \.diagram-nav, \.focus-chip, \.guided-views, \.flowforge-toast, \.no-print \{ display: none !important; \}/);
});
