...
> **Purpose:** Verify the core hypothesis: "Mermaid input + Claude layout + flowforge CSS" produces diagrams rated significantly better than stock Mermaid. See `../../ROADMAP.md` for the experiment design.
...
| **B** | Mermaid via `mmdc` + flowforge-style themeCSS — same dagre layout, flowforge color palette / font / background |
| **C** | Hand-placed flowforge HTML — Claude-assigned semantic classes + hand-placed coordinates + flowforge CSS |
...
Owner self-evaluation result: **C (flowforge hand-placed) looks good; A and B both don't look good.** B is not meaningfully better than A — swapping CSS without changing layout does not bridge the aesthetic gap. The experiment confirms all three pre-experiment reviews: layout is the product, not CSS.
...
- Mermaid input becomes a **SKILL.md prompt-engineering trick**: user pastes Mermaid, Claude reads the structure and lays out from scratch in flowforge style. No dagre, no parser, no auto-layout. This is already how flowforge works today (user describes → Claude draws), just with Mermaid as the input dialect instead of natural language.
