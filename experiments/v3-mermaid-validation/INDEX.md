# v3.0 Mermaid → FlowForge Validation — Source Diagrams
...
output-B-themed/— version (B): mmdc with flowforge-style themeCSS injected
output-C-flowforge/ — version (C): hand-ported to flowforge HTML
...
4. **Diagram 3's embedded `classDef`** — for fair comparison across A/B/C, version A and B will be rendered both with and without the embedded styling (so we can see whether moke-kit's hand-tuned colors already meet the bar before flowforge gets involved).
