...
How can FlowForge borrow the appeal of richer animated diagram products without weakening its strongest boundary: one offline, inspectable, deterministic HTML artifact built from authored semantics?
...
Cytoscape’s graph animation API separates ordered animation, viewport framing, pause/stop, and progress. Queued animations run in order; pause keeps current progress; stop removes work from queues; viewport fit/center is an explicit animation target. This reinforces a small ownership model for FlowForge: one finite route scheduler, one exact current position, one camera request, and immediate cancellation on manual navigation.
