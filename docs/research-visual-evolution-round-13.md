...
FlowForge does not add an animation timeline schema, moving nodes, editable keyframes, a graph runtime, or arbitrary path authoring. Story Trail is a truthful reading layer over compiled SVG semantics. If the source graph does not contain a relationship, the viewer does not invent one for visual continuity.
