...
Build **First-fold Proof Aperture**: make a meaningful slice of FlowForge's
...
FlowForge already has the richer product: three live proof choices, five typed
...
because it is visible proof, not because FlowForge must copy twelve styles or a
GIF renderer. Adding another interaction to an already dense viewer would be
imitating surface breadth. Making FlowForge's own stronger artifact visible is
...
[`flowforge/test/landing.test.mjs`](../flowforge/test/landing.test.mjs) verifies that
...
`docs/assets/flowforge-live-proof.gif`, backed by a receipt and focused tests.
...
**Adapt:** expose FlowForge's existing real HTML proof rather than copying a
...
**Adapt:** FlowForge already has a richer interactive artifact, so reveal it
...
preview.
**Adapt:** keep FlowForge's current three proof tabs and full artifact link.
...
**Adapt:** retain FlowForge's read-only, generated artifact and keyboard proof
tabs.
...
| Adapt | Reuse the three checked FlowForge artifacts and existing tab semantics. |
...
FlowForge does not need another capability to become more convincing in this
...
and let the canvas own attention—while preserving FlowForge's narrower, stronger
