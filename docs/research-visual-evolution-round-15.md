...
FlowForge does not add a timeline authoring schema, put HTML into the SVG export, duplicate the Guided Views toolbar inside embeds, loop a decorative status pulse forever, or guess labels from layout. The cue is a compact explanation of authored semantic state, not a second diagram system.
