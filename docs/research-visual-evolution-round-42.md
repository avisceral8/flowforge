...
The reason to do this now is measurable. FlowForge has accumulated a powerful
...
diagram (`flowforge/assets/template.html`, current lines 3642–3679). Its first
...
bounded solution (`flowforge/assets/template.html`, current lines 7327–7342):
...
`2.75rem` controls (`flowforge/assets/template.html`, current lines 2737–2755).
...
**Adapt:** compact FlowForge's inactive story surface while retaining its stronger
...
**Adapt:** FlowForge should preserve Play and every named chapter before
activation, then reveal its richer director after activation.
**Skip:** hiding all navigation, requiring fullscreen, or reducing FlowForge to
only Previous/Next.
...
**Adapt:** use FlowForge's existing semantic state, not a new host configuration,
...
FlowForge does not need another diagram feature to look richer. It needs its
...
composition contract exposes a real gap in FlowForge's uneven cross-renderer
