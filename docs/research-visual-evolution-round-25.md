...
FlowForge already has optional trace animation, one-hop Intent Trace, directed
...
- [`template.html`](../flowforge/assets/template.html) already clones authored edge
...
FlowForge can borrow this invariant directly with far less machinery.
...
FlowForge's self-contained HTML viewer can apply the preference directly and should
...
removal decide when motion exists. FlowForge does not need G6's renderer, extension
...
For FlowForge's smaller fixed SVGs, the simpler equivalent is stronger: create at
...
The safer FlowForge grammar is a short one-shot directional pulse, with the static
...
12. Focused contract tests cover exact-edge ownership, direction, finite timing,
    ...
    FlowForge suite and Proof Lab remain green.
...
and zero new runtime architecture. That is enough movement to make FlowForge feel
