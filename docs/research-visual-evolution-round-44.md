...
lets FlowForge tighten polished examples without rejecting legitimate engineering
topologies.
...
`flowforge/renderers/shared/layout-report.mjs` serializes component and connection
geometry only for architecture. `flowforge inspect` and `validate --layout-json`
...
of trusting coordinate equality; use FlowForge's real corpus to calibrate warning
budgets.
...
**Adapt:** only border-*collinearity* is universally wrong in FlowForge today;
...
- Keep shared-source/target channels measurable and non-blocking until FlowForge
...
**FlowForge tells an author whether a diagram is safe, whether it is polished,
...
| `composition/stacked-arrowhead` | multiple business arrowheads at the same endpoint coordinate | warning | warning | Fireworks rejects it, but FlowForge lacks port-offset controls today |
...
After explicit `port`, `junction`, and route-role semantics exist, FlowForge can
...
- `flowforge validate <type> <input> --json` returns `checks` plus
...
Round 44 is successful when FlowForge can truthfully say:
