...
FlowForge's desired quality bar, or an aggregate score that cannot name a route.
...
to routing quality.
**Adapt:** compute FlowForge's `cross-container` class from typed structural
...
**Adapt:** use discrete explainable FlowForge classes instead of exposing router
...
**Adapt:** make ordinary FlowForge sequence messages an exact renderer invariant
...
**Adapt:** reuse FlowForge's existing small `role` vocabulary instead of adding a
...
**FlowForge keeps primary routes direct, gives necessary detours an explicit
...
The 8px terminal threshold is an FlowForge adaptation below the current 13px
...
- Desktop/mobile have no horizontal overflow or clipped receipt text; focus
  targets remain at least 44px; console contains no FlowForge runtime errors.
