...
How can FlowForge become more useful in meetings and shared reviews without turning a diagram schema into a presentation-authoring system?
...
FlowForge already has the more valuable primitive: typed named views over stable semantic IDs. Round 7 therefore adds a **Presentation Stage around existing views**, not a second slide schema.
...
A slide DSL would duplicate titles, sequencing, and focus data already present in `meta.views`, introduce another validation surface, and encourage authored presentations to drift from the architecture source. A viewer-only stage makes every existing diagram more useful while preserving FlowForge's single-file, zero-runtime-dependency output.
