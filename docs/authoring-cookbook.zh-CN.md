...
FlowForge 首先是面向 Agent 的 Skill。普通用户只需向支持 Skill 的 Agent 描述想要的图；不必学习 Schema，也不必自己运行 `validate`、`inspect` 或 `deliver`。

下面的手工流程面向集成、贡献和排错。命令假设你位于仓库的 `flowforge/` 目录，或已经安装好的 FlowForge Skill 根目录。

## 1. 检查安装

FlowForge 要求 Node.js 18 或更高版本。开始编图前先运行 doctor：
...
```bash
node bin/flowforge.mjs guide "展示带 Redis 缓存未命中的 API 请求" --json --lang zh
```
...
每个源文件都需要 `schema_version`、`diagram_type`、`meta.title`，以及对应 Renderer 要求的结构数组。精确字段和允许值请阅读 [Schema 说明](../flowforge/schemas/README.md)。
...
FlowForge 会校验 Git 远端、commit、blob 和请求的代码行。无法验证仓库或版本时，不要添加源码证据。
...
视觉复核状态和交付回执字段请参阅 [交付契约](../flowforge/references/delivery-contract.md)。编图不变量和有上限的修复循环请参阅 [Skill 契约](../flowforge/SKILL.md)。
