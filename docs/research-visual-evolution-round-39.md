...
FlowForge's story system can now explain the present unusually well. Story Follow
...
([viewer implementation](../flowforge/assets/template.html)).
...
node
([Story Follow contract](../flowforge/test/story-follow-camera.test.mjs)).
...
motion layer would compete with surfaces FlowForge already has. Story Horizon
...
**Adapt:** FlowForge should create expectation with one static immediate-next
...
**Adapt:** unlike a strict reveal, FlowForge should retain its stable topology and
...
**Adapt:** reuse FlowForge's existing `forward`, `reverse`, `multiple`, and
...
and composition semantics in FlowForge's source schema.
...
motion, and preserves FlowForge's strongest product boundary—one stable,
