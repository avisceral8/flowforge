...
Build **Settled Flow**: an ordinary trace-enabled FlowForge artifact should run
...
FlowForge already has Story Trail, Story Beats, Story Follow Camera, Story
...
`flowforge/assets/template.html` currently gives trace edges and nodes:

```css
animation: flowforge-edge-flow 2.4s linear infinite;
animation: flowforge-node-pulse 3.6s ease-in-out infinite;
```
...
source topology.
**Adapt:** FlowForge can reuse its existing CSS markers and six-second WebM
...
**Adapt:** FlowForge's Story features already own deliberate step navigation, so
...
model.
**Adapt:** treat FlowForge animation state as viewer-only; authored classes and
...
legible settled topology, with source geometry and meaning preserved. FlowForge
