# FlowForge 下一增长切片研究：Last-Good Live Preview

日期：2026-07-22（Asia/Shanghai）
FlowForge 基线：`main@440e16d`；开放中的 [PR #44](https://github.com/tt-a1i/archify/pull/44) `@212639b` 已加入 opt-in Editable Source Capsule，五项远端检查全绿。**本建议只依赖 `main` 已有的 Atomic Delivery 与 Verified Open，可从 `main@440e16d` 独立落地，不以 PR #44 合并为前置。**
...
```bash
flowforge preview <type> <input.json> \
  [--quality standard|showcase] [--no-open]
```

它在仅监听 `127.0.0.1` 的随机端口打开一个桌面预览页；输入 JSON 变化后，先走 FlowForge 现有 renderer、composition gate 与 artifact checker，**只有完整通过才刷新浏览器**。非法或半写入输入只显示精确诊断，上一份已验证图继续留在画布上。修复后自动恢复。

这是当前最合适的“好用 + 稳定 + 可展示”切片：D2 和 Mermaid Live Editor 已证明实时反馈是文本制图的核心体验；而 FlowForge 此前推迟 preview 所缺的 Atomic Delivery 与 Verified Open 前置条件现在已经在 `main` 具备。它改善的是作者从“改 JSON”到“看见可信结果”的循环，不再给已经很丰富的成品 Viewer 增加一个控件，也不读取或依赖 PR #44 的 Source Capsule。
...
- FlowForge 已吸收这条纪律：三 preset 同拓扑、Semantic Sigils、有限 motion、确定性/感知式交付门。因此下一步不应再扩 style catalogue。
...
- 这使“Node → source evidence”值得进入候选，但 FlowForge 若没有 revision、private repo 与 opt-in privacy 合同，不能只复制可点击外观。

### draw.io MCP：编辑回路很顺，但其编辑器/格式矩阵不是 FlowForge 的边界
...
- PR #44 已为 FlowForge 提供更符合自身 IR 的 Source JSON 交接；再做 draw.io XML round-trip 会形成第二套真相和编辑器依赖。
...
- 它的 README 同时列出 VS Code、Vim/Neovim、Emacs 与 MCP 集成。[README](https://github.com/markmap/markmap/blob/99fc93e6efd4a1df01260232d818fb57955d71df/README.md#L1-L27) 对 FlowForge 的当前启发是保持 preview 与生成 artifact 解耦，不是立刻做 mindmap 或插件矩阵。

## FlowForge 已有 / 缺口映射

| 竞品中的有效模式 | FlowForge 当前状态 | 真正缺口 |
|---|---|---|
...
4. Preview shell 与被预览 artifact 分离。artifact 仍是普通自包含 FlowForge HTML；preview runtime、reload token、错误 banner、source path 和端口都不能写进 canonical HTML/SVG/图片/WebM。
...
- **第四/第五 preset、12-style 追数、vendor icon catalogue**：FlowForge 已有三种同拓扑视觉语言和 sigils；新增目录会成倍扩大主题、导出、动效与截图矩阵，却不缩短创作循环。
...
- **Mermaid 式压缩源码 share URL / 托管保存**：需要 hosted decoder、URL/XSS/长度、隐私与持久化合同；FlowForge 的一个 HTML 文件已经是更强的离线分享边界。
...
- **依赖原生 file watcher 或 WebSocket 作为唯一真相**：D2 源码已经展示其复杂度。FlowForge 应用内容摘要轮询 + debounce + generation token，server 只做本地预览，不引入新 npm 依赖。
