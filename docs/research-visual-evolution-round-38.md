...
look?” but not always “what happened here?” FlowForge already owns the facts needed
...
FlowForge has already solved the surrounding navigation layers:
...
**Adapt:** FlowForge should express semantic specificity as exact, readable beat
...
project, not general facts FlowForge can safely infer from its five typed IRs.
...
workspace navigation model. FlowForge's full static truth and local framing are
...
**Adapt now:** derive a concise explanation only from FlowForge's existing edge
...
**Adapt:** retain FlowForge's current pause-on-focus behavior and Live/Still owner.
...
| Per-diagram packet/train/ECG motion signatures | High spectacle | Low chrome, very high runtime surface | Needs explicit semantics FlowForge does not currently own | **Adapt later** |
...
feature has violated FlowForge's trust boundary. Fail closed to relation class,
