...
FlowForge can already restore a named chapter, one focused node, one authored route,
...
[current shared viewer source](../flowforge/assets/template.html)).
...
resolved stop inside that projection. On load, FlowForge rebuilds the current node,
...
[viewer URL writers and readers](../flowforge/assets/template.html)):
...
([focus writer](../flowforge/assets/template.html),
[guided-view writer](../flowforge/assets/template.html),
[route writer](../flowforge/assets/template.html),
[Lens writer](../flowforge/assets/template.html)).
...
[current Story Beat implementation](../flowforge/assets/template.html)).
...
([Semantic Camera and export implementation](../flowforge/assets/template.html)).
...
**Adapt:** FlowForge's exact Story moment needs one narrower coordinate inside that
...
**Adapt:** use a stable node ID instead of a numeric fragment index. FlowForge node
...
coordinates into an FlowForge link. In React Flow those values are the editable
flow; in FlowForge the JSON IR and compiled SVG are canonical, and runtime state
...
breakpoint, embed size, browser zoom, or Presentation chrome. FlowForge can instead
derive an appropriate camera from the same semantic chapter nodes on the current
device. This is an FlowForge design inference based on React Flow's explicitly
pixel/zoom viewport model and FlowForge's current semantic camera implementation
([React Flow viewport definition](https://reactflow.dev/learn/concepts/terms-and-definitions#viewport),
[FlowForge camera source](../flowforge/assets/template.html)).
...
**Adapt:** FlowForge's view is already the board and its existing stable node ID is
...
**Adapt:** FlowForge can go one useful step further and document the story beat as a
...
| Tool | Stable semantic address | Exact incremental position | Raw viewport snapshot | Embed/autoplay meaning | FlowForge judgment |
|---|---|---|---|---|---|
...
| FlowForge now | `focus`, `route`, `lens`, `view` | Beat is in memory only | Derived camera, no URL snapshot | `play=1` is bounded chapter scope | Add `view + beat`, preserve everything else |
