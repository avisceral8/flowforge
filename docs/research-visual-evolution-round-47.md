...
FlowForge already has stronger semantic exploration, guided stories, export, and
...
What can FlowForge learn from `fireworks-tech-graph` without copying its breadth
...
3. The same reference contains a large product/brand catalogue. That breadth is
   ...
   matching responsibilities that FlowForge does not need for its core promise.
...
## Decision: FlowForge Semantic Sigils
...
## Why this adaptation fits FlowForge
...
- Twelve generic selectable skins: FlowForge keeps three coherent presets.
...
- Style-specific looping signatures for every scene: FlowForge already uses
