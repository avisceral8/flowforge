...
FlowForge now has enough good motion primitives. Its next visual-quality problem is
...
- the current `codex/flowforge-visual-evolution` worktree, not an older package;
...
- the existing FlowForge boundary: one deterministic self-contained HTML artifact,
...
[header and preset styling](../flowforge/assets/template.html#L519-L632),
[Radar styling](../flowforge/assets/template.html#L705-L752), and
[motion rules](../flowforge/assets/template.html#L2635-L2936).
...
- [ambient trace pause selectors](../flowforge/assets/template.html#L2933-L2936);
- [manual Story start](../flowforge/assets/template.html#L5467-L5483);
- [chapter activation and Semantic Camera reveal](../flowforge/assets/template.html#L5536-L5555).
...
See the current [reduced-motion block](../flowforge/assets/template.html#L2998-L3047)
...
may throttle them, but FlowForge has not expressed an application invariant.
...
### The existing `FlowForge.motion` name is already taken

`FlowForge.motion` is the public WebM recording surface with `canRecord()` and
`recordWebm()`. A governor must not replace or silently change this API. Use a
distinct owner such as `FlowForge.motionGovernor` and keep explicit motion export
separate from live-viewer preference.

See [the current recording implementation](../flowforge/assets/template.html#L3920-L4025)
and [animation contract tests](../flowforge/test/animation.test.mjs).
...
WAI Technique G186 is unusually direct for FlowForge: the control must be
...
It is Level AAA, but it matches FlowForge's stated quality target and existing
...
FlowForge's interactive artifact can offer reader control that a GIF cannot.
...
second persistent camera model. FlowForge's current 480ms bounded fit is enough.
...
**Skip now:** adding this extra movement before FlowForge can guarantee that
...
Keep the existing `FlowForge.motion.canRecord()` and
`FlowForge.motion.recordWebm()` export surface untouched.
...
8. `FlowForge.motionGovernor` exists without replacing or renaming
   `FlowForge.motion` WebM methods.
...
47. Explicit WebM recording remains available for trace artifacts through the
    existing `FlowForge.motion` API and contains no Governor UI/state.
