...
an interactive, counted **Semantic Lens** over the semantics FlowForge already
compiled—not a generic database-style filter builder.

## Current FlowForge evidence

- [`focusNodeAttrs()`](../flowforge/renderers/shared/cli.mjs) already emits a stable
...
- The viewer already owns temporary focus, route, story, camera, URL, print, and
  export state in [`template.html`](../flowforge/assets/template.html). Lens belongs
...
FlowForge should borrow that interaction grammar, not G6's plugin/canvas stack or
...
FlowForge already has the simpler SVG equivalent: set viewer-only match/context
...
set visible, and offer an obvious reset. FlowForge does not need nested property
...
FlowForge is a fixed-geometry technical-diagram compiler. A semantic lens should
...
It is excessive for FlowForge. The compiled diagram is the authoritative view;
...
authoring burden. It turns FlowForge's existing colors and node kinds from passive
