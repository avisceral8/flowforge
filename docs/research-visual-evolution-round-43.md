...
copy Fireworks' complete showcase budget into FlowForge. Instead, it should move
...
This is deliberately a correctness gate, not an auto-router. FlowForge keeps the
...
- `flowforge/renderers/shared/geometry.mjs`: `segmentIntersectsRect` already
  provides the shared segment/rectangle primitive;
- `flowforge/renderers/workflow/render-workflow.mjs`: the only renderer that
  currently expands unrelated node boxes by 2px and rejects a crossing;
- `flowforge/renderers/architecture/render-architecture.mjs`,
...
- `flowforge/scripts/check-render-output.mjs`: performs a separate, post-render
...
`flowforge/test/layout-rules.test.mjs` has a focused negative fixture for a
workflow edge crossing a non-endpoint node, and
`flowforge/test/geometry.test.mjs` directly tests the segment/rectangle
...
**Adapt:** begin with the one invariant FlowForge can infer exactly from its
...
quality profile, not an unconditional retrofit over FlowForge's existing
...
classes.
**Adapt:** FlowForge should hard-gate peer node boxes and explicitly exempt
...
**Adapt:** retain FlowForge's current 2px workflow clearance in the first shared
gate so the rollout fixes inconsistency without silently tightening every
existing layout.
**Skip:** claiming a global zero-crossing guarantee when FlowForge is not running
...
message.
**Adapt:** FlowForge's receipt should point directly to `via`, `route`,
...
FlowForge currently has no schema concept for a junction, shared corridor,
...
**FlowForge never publishes a relationship through an unrelated semantic node,
...
exempt because crossing them can be semantically necessary and FlowForge has no
...
4. `flowforge validate <type> <input>` and direct renderer invocation reject the
   same bad geometry.
...
| Adapt | Reuse FlowForge's exact authored route points and the existing 2px workflow clearance. |
...
the right improvement is the small, strict statement FlowForge can prove today:
