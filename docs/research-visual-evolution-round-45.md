...
numbers, while FlowForge does not yet serialize a route role rich enough to tell
...
FlowForge's current schemas; retain the three route budgets as metrics until
route-role calibration exists.
**Skip:** copying Fireworks' showcase numbers as immediate FlowForge failures or
...
**Adapt:** point to FlowForge's existing `via`, channel, side, and placement
...
- Border runs fail both FlowForge profiles; the Fireworks route-budget numbers
...
**A relationship may enter or leave a container, but FlowForge never lets the
...
because current FlowForge relationships target semantic nodes, not containers.
...
- Browser console logs contain no FlowForge runtime error.
