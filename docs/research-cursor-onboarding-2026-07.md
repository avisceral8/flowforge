# FlowForge first-class Cursor onboarding research

> Snapshot: 2026-07-23. Sources are limited to Cursor's official documentation and changelog, the official `vercel-labs/skills` repository and CLI, FlowForge's own issue/docs/tests, and first-party peer repositories.

## Decision

**Recommendation:** add Cursor as a first-class installation surface for the existing `flowforge` Skill. Do not fork `SKILL.md`, add a Cursor-only runtime, or call the work complete until one real Cursor Editor/CLI session discovers the Skill and produces a verified artifact.

Use an explicit agent, skill, scope, and copy mode in product copy:

```bash
# Individual use: available in every Cursor project
npx -y skills add tt-a1i/archify --skill flowforge --agent cursor --global --copy --yes

# Team/repository use: installed only in the current project
npx -y skills add tt-a1i/archify --skill flowforge --agent cursor --copy --yes
```

For a reproducible test fixture, pin the version that was verified in this snapshot:

```bash
npx -y skills@1.5.20 add tt-a1i/archify --skill flowforge --agent cursor --copy --yes
```
...
| Scope | Valid Skill roots | FlowForge entry |
|---|---|---|
| Project | `.agents/skills/`, `.cursor/skills/` | `<root>/flowforge/SKILL.md` |
| User/global | `~/.agents/skills/`, `~/.cursor/skills/` | `<root>/flowforge/SKILL.md` |
...
- The current FlowForge README names Claude, Codex CLI, and opencode, and its installation matrix does not include Cursor.[Current positioning](https://github.com/tt-a1i/archify/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/README.md#L11-L19) · [Current installation matrix](https://github.com/tt-a1i/archify/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/README.md#L223-L231)
- FlowForge requires Node.js 18 or newer, but its distributed renderer and validators do not require `npm install`.[Runtime contract](https://github.com/tt-a1i/archify/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/flowforge/package.json#L8-L13) · [Skill setup contract](https://github.com/tt-a1i/archify/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/flowforge/SKILL.md#L27-L33)
- Existing tests already simulate a copied installation with `node_modules` and repository-only tests removed, then exercise all five modes through `deliver` and run live preview.[Installed-copy fixture](https://github.com/tt-a1i/archify/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/flowforge/test/cli.test.mjs#L56-L69) · [Five-mode installed delivery](https://github.com/tt-a1i/archify/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/flowforge/test/cli.test.mjs#L357-L381) · [Installed preview](https://github.com/tt-a1i/archify/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/flowforge/test/cli.test.mjs#L383-L425)
...
npx -y skills@1.5.20 add tt-a1i/archify \
  --skill flowforge --agent cursor --copy --yes
```

Observed evidence:

1. `npx -y skills@1.5.20 list --agent cursor --json` reported one project-scoped `Cursor` installation at `.agents/skills/flowforge`.
2. `node .agents/skills/flowforge/bin/flowforge.mjs doctor` exited 0.
...
|---|---|---|---|
| Global | Current CLI resolves Cursor to `~/.agents/skills/flowforge`; Cursor officially scans that root. | A shell-capable Skill becomes available in every project; update timing is user-wide. | Default for an individual who wants FlowForge everywhere. Say exactly what scope it grants. |
| Project | Installs `.agents/skills/flowforge` plus `skills-lock.json` in the current repository. | The copied Skill is a substantial repository diff if committed, and can become stale relative to upstream. | Offer as the team/reproducible option, not as a silent default. |
...
- “Install FlowForge for Cursor with `--agent cursor`; use `.agents/skills/flowforge` for a project or a Cursor-supported global Skill root for user scope.”
- “The installed FlowForge package passes `doctor`, validation, and checked HTML delivery without `npm install`, provided Node.js 18+ is available.”
- “FlowForge uses the same portable `SKILL.md` and renderer contract across Cursor, Claude, Codex, and opencode.”
...
- “The global CLI command installs to `~/.cursor/skills/flowforge`.” The current implementation resolves Cursor as universal and may use `~/.agents/skills/flowforge`.
- “Cursor's Remote Rule UI imports FlowForge correctly from the bare repository URL.” Cursor documents the UI, but not nested Skill selection; FlowForge's actual Skill lives under `flowforge/`.
...
3. Reuse the same `flowforge/SKILL.md`; add no Cursor fork or Cursor-specific diagram behavior.
