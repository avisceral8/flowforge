...
FlowForge already compiles authored `guidedViews`, renders the active chapter's
...
[`FlowForge.guidedViews`](../flowforge/assets/template.html) parses the existing
`flowforge-guided-views-data` payload into an ordered `views` array. Every entry
already has:
...
direct live-SVG serialization as FlowForge's export implementation. FlowForge's
...
selection. FlowForge already has deliberate Play/Pause ownership and should not
expand it.
...
ambient rotation, or full-page layout. The FlowForge rail must work by explicit
...
or a current-viewport export option in this slice. The current FlowForge
...
3. Do not change `FlowForge.exporter`, export cleanup, canonicality receipts,
...
boundaries keeps the result attractive, stable and recognizably FlowForge.
