...
chapter, FlowForge should show which stable node IDs would **stay in focus**, **enter
...
No inspected primary source ships this exact FlowForge interaction as a package.
...
[`FlowForge.guidedViews`](../flowforge/assets/template.html#L5424-L6252):
...
is inspected. **Adapt:** FlowForge needs one preview generation over stable SVG IDs,
...
would duplicate rendering and hide the precise semantic delta FlowForge can provide.
...
matters. **Adapt:** derive membership from independent FlowForge `focus` arrays; do
...
10. Focus, hover, and Escape do not write `activeIndex`, `aria-current`,
    `aria-pressed`, URL/hash/history, `FlowForge.focus`, Story Trail data,
...
Keep this inside the existing template-level `FlowForge.guidedViews` module so all
