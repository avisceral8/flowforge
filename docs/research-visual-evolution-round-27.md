...
FlowForge already has two category surfaces:
...
> the existing Semantic Lens, and can that feel alive without turning FlowForge
...
[`FlowForge.semanticLens`](../flowforge/assets/template.html) currently:
...
- [`render-architecture.mjs`](../flowforge/renderers/architecture/render-architecture.mjs)
  ...
  bridge.
- [`render-workflow.mjs`](../flowforge/renderers/workflow/render-workflow.mjs)
...
- [`render-lifecycle.mjs`](../flowforge/renderers/lifecycle/render-lifecycle.mjs)
...
- [`render-sequence.mjs`](../flowforge/renderers/sequence/render-sequence.mjs)
  explains message/edge variants such as request and return. It is not a node
  category legend and must remain static.
- [`render-dataflow.mjs`](../flowforge/renderers/dataflow/render-dataflow.mjs)
...
of the live SVG for FlowForge's export path. FlowForge already has a stronger
canonical-clone boundary and should keep runtime decorations out of export.

This source is also a useful negative test: a legend can look like FlowForge's
...
The exact count badge is an FlowForge inference from the same category-to-ID
...
FlowForge must add keyboard, focus, touch, semantic state, and target-size behavior
...
This distinction maps cleanly to FlowForge:
...
For FlowForge's SVG-hosted legend rows, this means:
...
state. FlowForge should borrow it, with stricter limits:
...
- G6's exact category-to-element mapping, implemented through FlowForge's existing
...
- Edge-semantic legend lenses for Sequence and Data Flow, but only after FlowForge
...
3. Every visible count equals the corresponding value returned by
   `FlowForge.semanticLens.kinds()` after deduplication by `data-node-id`.
