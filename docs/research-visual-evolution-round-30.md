...
FlowForge already has the right substrate: one canonical SVG, stable semantic node
...
3.2 seconds ([template](../flowforge/assets/template.html#L5784-L5890)). Semantic
...
([camera](../flowforge/assets/template.html#L6083-L6252)). The Governor already has
replaceable token ownership and stale-release protection
([governor](../flowforge/assets/template.html#L4373-L4582)). SVG export clones the
canonical graph and strips viewer transforms and overlays
([serialization](../flowforge/assets/template.html#L3709-L3830)).
...
| Adapt | FlowForge needs no cloned views or heuristic matching: both chapters reference the same persistent `data-node-id` elements. |
...
6. `FlowForge.view.reveal()` should return a transaction `{ id, state, finished,
