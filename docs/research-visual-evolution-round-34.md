...
FlowForge already exposes authored relationships in every typed renderer and can
...
([relationship renderer contract](../flowforge/renderers/shared/cli.mjs),
[current Relationship Lens](../flowforge/assets/template.html),
[current relationship tests](../flowforge/test/relationship-lens.test.mjs)).
...
orientation interaction. FlowForge already has node focus, one-hop intent,
...
> Should an FlowForge reader be able to interact with an authored relationship
...
of 2026-07-20. Product conclusions and proposed FlowForge behavior are explicitly
marked as recommendations or inferences rather than vendor claims.

## Current FlowForge baseline
...
([shared renderer helper](../flowforge/renderers/shared/cli.mjs)). The current
...
([relationship renderer tests](../flowforge/test/relationship-lens.test.mjs)).
...
[shared node accessibility helper](../flowforge/renderers/shared/cli.mjs)).
...
[current relationship geometry helper](../flowforge/assets/template.html)).
...
row visible
([Relationship Lens implementation](../flowforge/assets/template.html),
[Relationship Lens tests](../flowforge/test/relationship-lens.test.mjs)).

The gap is consequently not “invent an edge inspector.” It is “let the line open
the inspector state FlowForge already has.” Reusing the existing panel is important
...
embed variants
([shared viewer chrome](../flowforge/assets/template.html)).
...
([preview and pulse implementation](../flowforge/assets/template.html),
[export-clean regression tests](../flowforge/test/relationship-lens.test.mjs),
[pulse regression tests](../flowforge/test/relationship-pulse.test.mjs)).
...
channel
([Motion Governor implementation](../flowforge/assets/template.html),
[Motion Governor tests](../flowforge/test/motion-governor.test.mjs)).
...
([focus, route, Lens, and Guided Story hash readers](../flowforge/assets/template.html),
...
([shared renderer helper](../flowforge/renderers/shared/cli.mjs),
[typed renderer examples](../flowforge/examples)). Therefore Round 34 should keep
...
**Adapt:** FlowForge is a viewer, so activation means “inspect/pin this authored
...
contract is clearer for FlowForge.
...
**Adapt:** FlowForge already has SVG and can create genuine focusable DOM targets,
...
**Adapt:** FlowForge needs one pinned relationship, not yFiles' editor command
...
radius. This is an FlowForge implementation recommendation based on the W3C and
...
FlowForge should borrow the focus-management technique, not claim that diagram
...
**Borrow later:** if FlowForge adds relationship links, use an author-controlled
...
| Should pin create new UI? | FlowForge already has an exact Relationship Lens row and is chrome-dense. | No. Reuse and contextualize the existing Lens. |
...
| Is current identity sufficient for a deep link? | Graphviz requires author-controlled unique IDs; FlowForge's key is a source-order number. | No. Keep the pin local in Round 34 and do not emit a relation URL. |
...
- do not add a schema field, migration, server, storage, dependency, or editor
  API; and
- expose at most a small read-only `FlowForge.relationship` receipt/API for testing
...
works with mouse, touch, and keyboard, and composes with FlowForge's existing
