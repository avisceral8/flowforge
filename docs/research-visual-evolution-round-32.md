...
FlowForge already turns each authored `meta.views[].focus` list into a visible Story
...
## Why this slice is real in current FlowForge

The implementation substrate already exists in
[`FlowForge.guidedViews`](../flowforge/assets/template.html):
...
`docs/gallery/artifacts/*.html`, using only `flowforge-guided-views-data` and exact
...
request one finite signal over one exact FlowForge path, while the static beat
...
**Adapt:** FlowForge already has an authored stable-ID node sequence, a fixed SVG,
...
state, and previous/next semantics. **Adapt:** FlowForge's authored `focus` order is
...
recomputation, and any claim that prior FlowForge beats mutate the graph.
...
14. Expose a read-only deterministic receipt such as
    `FlowForge.guidedViews.beat()` returning `{index, total, nodeId, relation,
