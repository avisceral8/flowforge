...
- [`relationshipTokenKind()` and `relationshipTokenGeometry()`](../flowforge/assets/template.html)
...
- [`storyStep()` and `pulseStoryStep()`](../flowforge/assets/template.html) build
  ordered Story Beats and animate the generic Story Trail flow;
- [`storyMotionAllowed()`](../flowforge/assets/template.html) rejects every embed,
  even when the URL explicitly requested the existing `?embed=1&play=1` share
  contract;
- [`followStoryStep()`](../flowforge/assets/template.html), by contrast, correctly
...
- [shared Story Beat implementation](../flowforge/assets/template.html)
...
**Adapt:** FlowForge already has stronger stable `data-edge-key` / optional
...
**Adapt:** one carrier per selected Story Beat, with FlowForge's existing cleanup
and motion governor.
