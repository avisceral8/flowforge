...
The question-first guide defined 11 useful scenario recipes, but only five had a matching live artifact. That weakened the promise: users could receive good advice without seeing whether FlowForge could actually render the recommended result.
...
FlowForge keeps its smaller boundary—one self-contained diagram rather than a persistent architecture model—but adopts the product lesson: a scenario recommendation should lead to a named, shareable proof instead of a generic gallery homepage.
...
FlowForge's equivalent is intentionally geometry-stable: each proof keeps one typed JSON graph and one SVG layout, then uses three bounded `meta.views` to focus successive parts. Playback changes emphasis, never the underlying coordinates.
