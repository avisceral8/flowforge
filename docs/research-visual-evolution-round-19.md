...
FlowForge has strong committed exploration after a reader clicks a node: Semantic
...
Can FlowForge make the diagram feel immediately explorable without adding another
...
`clickNode`. The event payload includes the stable node ID. FlowForge should keep
...
and notes that labels or details can be delayed until tap/mouseover. FlowForge
...
triggered it. FlowForge already has the equivalent stable identity in
