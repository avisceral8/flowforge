...
FlowForge already has the major viewer primitives that modern graph tools use to
...
([Guided Story implementation](../flowforge/assets/template.html),
[Semantic Camera implementation](../flowforge/assets/template.html)).
...
first-glance hierarchy and reading comprehension—and what should FlowForge add
after its existing Radar, Camera, Reading Depth, Guided Story, Route Journey,
Motion Governor, and presentation features?

This note uses:

1. the current FlowForge worktree and generated Proof Lab artifacts;
...
4. explicit product inferences where a source supplies a primitive rather than
   an FlowForge-specific recommendation.
...
## Current FlowForge baseline: do not rebuild solved layers
...
([Semantic Radar implementation](../flowforge/assets/template.html),
[Semantic Radar tests](../flowforge/test/semantic-radar.test.mjs)).
...
([shared viewer CSS and camera](../flowforge/assets/template.html)).
...
([Reading Depth implementation](../flowforge/assets/template.html),
...
position means “default opening.” FlowForge should not infer cold-open intent from
...
([Story Beat tests](../flowforge/test/story-beat-navigator.test.mjs),
...
([camera implementation](../flowforge/assets/template.html),
[Route Journey tests](../flowforge/test/route-journey.test.mjs)).
...
FlowForge's existing camera. It should not store a React Flow viewport or make the
...
**Adapt:** FlowForge should not queue camera moves ahead of the reader. The Story
...
**Adapt:** FlowForge already has camera transaction IDs and settlement receipts.
...
**Adapt:** FlowForge's authored order is already `meta.views[].focus`; its Story
...
**Adapt:** FlowForge should preserve one SVG and express past/current/pending by
...
> Play or select a Story Beat, and FlowForge follows that exact authored moment
...
Call the already-public `FlowForge.view.reveal(ids, options)` from a small
...
- one shared Story-to-Camera request path in
  [`flowforge/assets/template.html`](../flowforge/assets/template.html);
- one story-owned caller of the existing public `FlowForge.view.reveal` method,
...
**Adapt** them into a Story Follow Camera over FlowForge's existing stable IDs,
...
observed legibility failure at the exact point FlowForge's narrative becomes most
