...
Date: 2026-07-23
Decision: build one bounded Architecture-only slice; do not widen FlowForge into a code indexer.
...
- Fireworks Tech Graph wins attention through visual breadth and motion, but its current public surface already overlaps FlowForge's offline HTML/export strengths ([README at `50c819d`](https://github.com/yizhiyanhua-ai/fireworks-tech-graph/blob/50c819d68fd4fee330b3010988cd13e98b678d44/README.md#L136-L153)). Copying more styles is lower-value than making FlowForge's existing semantic exploration more trustworthy.
...
- `flowforge render|validate|deliver|preview ... --repo-root <path>` is required when evidence exists;
...
It closes a concrete trust gap without disturbing the stable renderer, geometry, export, motion, or interaction contracts. It also gives FlowForge a better growth sentence than “more diagram styles”: **turn a high-level architecture map into a revision-pinned doorway to the code that proves it.**
