# FlowForge visual evolution research — round 2

Research date: 2026-07-19 (Asia/Shanghai)

## Executive decision

FlowForge should absorb **viewer intelligence**, **stable semantic identity**, and
...
associated with graph canvases without turning FlowForge into one.

## Round 2 implementation status

The `codex/flowforge-visual-evolution` branch now implements the first candidate
...
checks keep it inside FlowForge's read-only viewer boundary.
...
- Motion is deliberately constrained by semantic roles and scene contracts,
  but its optional GIF path is a substantially heavier stack than FlowForge's
...
- a scenario suitability guide that tells users which FlowForge renderer and
...
- drag, resize, connect, lasso, whiteboard, collaborative editing, and a React
  runtime. Those solve canvas-authoring problems outside FlowForge's boundary.
...
- Its default visual renderer uses the browser Canvas API. That enables large,
  rich graphs, but Canvas does not itself give FlowForge the semantic DOM and
...
- Its extension surface includes minimaps, fisheye, tooltips, history, timebars,
  ...
  but also evidence that the full engine is much broader than FlowForge needs
...
- embedding G6 or its plugin ecosystem. FlowForge can implement the small,
...
- the conceptual split between an overview, alternate scenario, and ordered
  walkthrough; use it to shape FlowForge recipes and examples.
...
- Mermaid is embeddable through ESM/CDN and offers a smaller “tiny” build, but
  the full package has a broad runtime dependency graph. FlowForge should copy
...
- The JavaScript distribution supports Web Workers and ships a browser-ready
  ...
  FlowForge's current renderer-specific geometry
...
| Capability | Best evidence | FlowForge interpretation |
|---|---|---|
...
> FlowForge compiles typed technical intent into a deterministic, validated,
