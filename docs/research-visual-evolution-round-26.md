...
FlowForge should add one **finite, source-to-target Directional Flow Pulse** to the
...
- [`template.html`](../flowforge/assets/template.html) already resolves each
...
**FlowForge inference:** borrow the one bright head, exact path direction, immutable
...
**FlowForge inference:** Relationship Preview entry is the owning lifecycle stage.
...
**FlowForge inference:** keep the original edge as the hit target and semantic
...
background assumption. FlowForge already has a better hit target—the named
...
**FlowForge inference:** a CSS-only one-shot is smaller than a queued animation
...
An FlowForge relationship row is already a button. The HTML Standard forbids
