...
const flowforgeCli = path.join(repoRoot, 'flowforge/bin/flowforge.mjs');
const COLLECTIONS = {
...
const PROMPT_CONTRACT_PHRASES = [
  ...
  'Use the bundled FlowForge CLI to validate and repair the candidate when shell access is available.',
...
function validateCandidate(benchmarkCase, candidateFile) {
  const validated = spawnSync(process.execPath, [
    ...
    cwd: path.join(repoRoot, 'flowforge'),
    encoding: 'utf8',
  });
