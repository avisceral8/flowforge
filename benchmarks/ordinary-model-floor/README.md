...
This benchmark measures a narrow product question: can an ordinary coding agent produce an FlowForge diagram that is usable on attempt 1, without a human repairing the JSON?
...
2. The real FlowForge CLI passes `validate --quality showcase --json`.
...
Every compared configuration must use the same prompt, the same repository commit, the same FlowForge skill and schema, the same time limit, identical tool access, and a clean candidate output path. Record the exact agent and model names. One complete agent invocation is attempt 1; the resulting `benchmark-candidate.json` is frozen when the invocation ends. The agent may use the bundled FlowForge CLI to validate and repair its candidate during that invocation; the external harness independently revalidates the frozen file and remains the final authority. Preserve that unedited candidate and permit no post-hoc edits, including human edits, before verification.
...
The harness deliberately does not launch model providers. The external runner owns authentication, model selection, timeouts, prompt delivery, and raw transcript retention. This keeps provider code and secrets out of FlowForge while making the artifact checks deterministic.
