...
const TYPES = new Set(['bpmn', 'architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'journey', 'infoflow', 'stakeholder-map', 'capability', 'okr-tree', 'north-star', 'growth-loop', 'launch-plan', 'feedback-pipeline']);

function usage() {
  return `Usage:
  flowforge render <type> <input.json> [output.html] [--quality standard|showcase] [--repo-root path]
  flowforge compare bpmn <base.json> <head.json> [output.html] [--receipt path] [--json] [--quality standard|showcase] [--repo-root path]
  flowforge deliver <type> <input.json> [output.html] [--json] [--open] [--quality standard|showcase] [--repo-root path]
  flowforge preview <type> <input.json> [output.html] [--no-open] [--quality standard|showcase] [--repo-root path]
  flowforge validate <type> <input.json> [--json] [--layout-json] [--quality standard|showcase] [--repo-root path]
  flowforge inspect <type> <input.json>
  flowforge check <output.html>
  flowforge visual-check <output.html> [--json]
  flowforge guide [scenario or question] [--json] [--lang en|zh]
  flowforge brands [name, alias, domain, or category] [--json]
  flowforge brands capture <url> [--json]
  flowforge examples
  flowforge doctor
  flowforge demo [output-directory]

Types:
  bpmn, architecture, workflow, sequence, dataflow, lifecycle,
  journey, infoflow, stakeholder-map, capability, okr-tree, north-star, growth-loop, launch-plan, feedback-pipeline
`;
}
...
function rendererEnv(quality, repoRoot, diagnosticJson = false) {
  return {
    ...(quality ? { FLOWFORGE_QUALITY_PROFILE: quality } : {}),
    ...(repoRoot ? { FLOWFORGE_REPO_ROOT: repoRoot } : {}),
    ...(diagnosticJson ? { FLOWFORGE_DIAGNOSTIC_FORMAT: 'json' } : {}),
  };
}
...
async function commandCompare(args) {
  ...
  if (type === 'bpmn') {
    await commandCompareBpmn(options, baseInput, headInput, requestedOutput, resolveOutputPath);
    return;
  }
  ...
  try {
    ...
    reportCompareFailure({ json: options.json, stage: 'prepare', error: 'Architecture compare runtime is unavailable.', code: 'delta/runtime-missing', details: { reason: error.message, supportedFixes: ['install the complete FlowForge skill package'] } });
    return;
  }
  const {
  ...
  try {
    ...
    const outputDiagnostic = error.flowforgeDiagnostics?.[0];
  ...
  try {
    stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.flowforge-compare-'));
  } catch (error) {
...
function sourceEvidenceFromArtifact(artifact) {
  ...
  const match = html.match(/<script id="flowforge-source-evidence-data" type="application\/json">([\s\S]*?)<\/script>/);
...
async function commandCompareBpmn(options, baseInput, headInput, requestedOutput, resolveOutputPath) {
  if (!baseInput || !headInput || options.positional.length > 4) fail(usage());
  const basePath = path.resolve(baseInput);
  const headPath = path.resolve(headInput);
  let outputPath;
  try {
    ({ outputPath } = resolveOutputPath({
      requestedOutput,
      defaultOutput: 'bpmn-delta.html',
      inputPaths: [basePath, headPath],
    }));
  } catch (error) {
    reportCompareFailure({ json: options.json, stage: 'prepare', error: error.message, code: 'output/path-resolution' });
    return;
  }
  const receiptPath = options.receipt
    ? path.resolve(options.receipt)
    : path.join(path.dirname(outputPath), path.basename(outputPath, path.extname(outputPath)) + '.receipt.json');
  let receipt;
  try {
    const deltaRuntime = await import(pathToFileURL(path.join(skillRoot, 'delta/bpmn-delta.mjs')).href);
    receipt = deltaRuntime.writeBpmnDelta({ baseInput: basePath, headInput: headPath, outputHtml: outputPath, receiptPath });
  } catch (error) {
    reportCompareFailure({ json: options.json, stage: 'delta', error: error.message, code: 'delta/bpmn' });
    return;
  }
  if (options.json) {
    console.log(JSON.stringify({ schemaVersion: 1, ok: true, command: 'compare', type: 'bpmn', output: outputPath, receipt: receiptPath, facts: receipt }, null, 2));
  } else {
...
async function commandDeliver(args) {
  ...
  try {
    ...
    reportDeliveryFailure({
      ...
      error: error.message,
      diagnostics: error.flowforgeDiagnostics || [diagnostic({
  ...
  let stagingDirectory;
  try {
    stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.flowforge-delivery-'));
  } catch (error) {
...
async function commandDoctor() {
  ...
  if (validatorsExist) {
    try {
      ...
      validatorsValid = [...TYPES].every((type) => typeof module[type.replaceAll('-', '_')] === 'function');
    } catch {
  ...
  console.error(`\nFlowForge is not ready: ${problems.join('; ')}.`);
  process.exitCode = 1;
}
...
async function commandGuide(args) {
  ...
  // Business-type router: when the question clearly matches a registered
  // FlowForge business type (bpmn or a canvas type), prefer it over the
  // upstream technical scenarios.
  let registry;
  try {
    registry = await import(pathToFileURL(path.join(skillRoot, 'types/registry.mjs')).href);
  } catch {
    registry = null;
  }
  if (registry) {
    const down = query.toLowerCase();
    let best = null;
    let bestScore = 0;
    for (const entry of registry.loadTypeRegistry().guideSignals()) {
      let score = 0;
      for (const keyword of [...(entry.keywords || []), ...(entry.signals || [])]) {
        if (down.includes(keyword.toLowerCase())) score += keyword.length;
      }
      if (score > bestScore) {
        bestScore = score;
        best = entry;
    ...
    if (best && bestScore >= 6) {
      const registryResult = {
        ...
        lang: lang || 'en',
        query,
        confidence: bestScore >= 14 ? 'high' : 'medium',
        matchedSignals: [best.id],
        recommendation: { id: best.id, type: best.id, title: best.label, question: `What does this ask map to? The FlowForge type \`${best.id}\`.`, useWhen: best.conditions?.useWhen, avoidWhen: best.conditions?.avoidWhen },
        alternatives: [],
      };
      console.log(json ? JSON.stringify(registryResult, null, 2) : `Recommendation: ${best.label}  [${best.id}]\nConfidence: ${registryResult.confidence}\nUse when: ${best.conditions?.useWhen}\nAvoid when: ${best.conditions?.avoidWhen}`);
      return;
    }
  }
...
async function commandBrands(args) {
  ...
  if (positional[0] === 'capture') {
    if (positional.length !== 2) fail('Usage: flowforge brands capture <url> [--json]');
  ...
  if (json) {
    console.log(JSON.stringify({
      ...
      count: marks.length,
      marks,
      fallback: 'Run "flowforge brands capture <url> --json", then use the returned digest-pinned brand value.',
    }, null, 2));
    return;
  }
  if (!marks.length) {
    console.log(`No built-in brand matched "${query}". Run "flowforge brands capture <url> --json", then use the returned digest-pinned brand value.`);
    return;
  }
...
function commandDemo(args) {
  ...
  const output = path.join(outputDirectory, 'flowforge-demo.html');
  ...
  console.log('  flowforge render architecture <input.json> <output.html>');
}
...
function commandValidate(args) {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-validate-'));
