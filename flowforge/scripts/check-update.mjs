...
function notification(localRelease, candidate) {
  return {
    ...
    summary: `FlowForge ${candidate.version} is available; see the official release notes for details.`,
...
function defaultCacheDirectory() {
  ...
  if (process.platform === 'win32') {
    ...
    return path.join(localData && path.isAbsolute(localData) ? localData : path.join(homeDirectory, 'AppData', 'Local'), 'flowforge-skill');
  }
  if (process.platform === 'darwin') return path.join(homeDirectory, 'Library', 'Caches', 'flowforge-skill');
  const xdgCache = process.env.XDG_CACHE_HOME;
  return path.join(xdgCache && path.isAbsolute(xdgCache) ? xdgCache : path.join(homeDirectory, '.cache'), 'flowforge-skill');
}
...
async function runCli() {
  if (process.env.FLOWFORGE_UPDATE_CHECK_DISABLED === '1') return silent('disabled');
