...
export function compareArchitecture(base, head, evidence = {}) {
  ...
  if (!shared.length) {
    fail('delta/no-shared-component-id', 'The snapshots share no component id, so FlowForge cannot prove that they describe the same system.', {
...
<script id="flowforge-compare-receipt" type="application/json">${safeJson(receipt)}</script>
<script>(()=>{
  ...
  const receiptNode = document.querySelector('#flowforge-compare-receipt');
  ...
  function validateReview() {
    try {
      if (document.querySelectorAll('#flowforge-compare-receipt').length !== 1 || !receiptNode) return false;
  ...
  function recordExport(format, blob, dimensions) {
    document.documentElement.dataset.flowforgeDeltaExport = JSON.stringify({
      ok: true,
      format,
      bytes: blob.size,
      mime: blob.type,
  ...
  window.FlowForge.deltaExport = { canonicalSvg: canonicalDeltaSvg, shareCard, exportSvg: exportCanonicalSvg, downloadShareCard };
  window.FlowForge.exportMenu = {
    shareCard,
    run(format) {
...
export function validateArchitectureDeltaHtml(html, receipt) {
  ...
  if ((html.match(/id="flowforge-compare-receipt"/g) || []).length !== 1) failures.push('expected exactly one embedded compare receipt');
  ...
  if (!html.includes('id="export-svg"') || !html.includes('id="share-card"')
    || !html.includes('window.FlowForge.deltaExport = { canonicalSvg: canonicalDeltaSvg, shareCard')) {
