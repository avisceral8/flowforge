...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-story-beat-navigator-'));
...
test('one generation-owned scheduler resumes remaining dwell and finite exact-edge signals never loop', () => {
  ...
  assert.match(template, /animation: flowforge-story-flow 0\.78s linear 1 both/);
  assert.doesNotMatch(template, /flowforge-story-flow 0\.78s linear infinite/);
