...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-route-journey-'));
...
test('motion, camera, layered Escape, mobile, print, and embed boundaries stay explicit', () => {
  assert.match(template, /FlowForge\.motionGovernor\.capable === true[\s\S]*?!FlowForge\.motionGovernor\.isPaused\(\)/);
  assert.match(template, /FlowForge\.motionGovernor\.claim\('route'/);
  assert.match(template, /reason: 'route-journey',[\s\S]*?maxScale: 1\.65,[\s\S]*?padding: 64,[\s\S]*?duration: 360/);
  assert.match(template, /FlowForge\.routeProbe\.pauseJourney\(\{ preserveElapsed: true, reason: reason \|\| 'manual' \}\)/);
  ...
  assert.match(template, /FlowForge\.routeProbe\.escape\(\{ restoreFocus: true \}\)/);
