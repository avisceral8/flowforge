...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-motion-governor-'));
...
test('all renderers inherit one viewer-only Live/Still Motion Governor', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /FlowForge\.motionGovernor = \(function \(\)/, mode);
...
test('reader pause is persistent, explicit, and reduced-motion aware', () => {
  ...
  assert.match(html, /var STORAGE_KEY = 'flowforge-motion'/);
  ...
  assert.match(html, /FlowForge\.guidedViews\.isPlaying\(\)[\s\S]*?FlowForge\.guidedViews\.pause\(\)/);
...
test('strong semantic intent receives the single motion budget', () => {
  ...
  assert.match(html, /animation: flowforge-route-probe-flow 1\.1s[\s\S]*?1 both/);
  assert.doesNotMatch(html, /flowforge-route-probe-flow[^;]*infinite/);
});
