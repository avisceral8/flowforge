...
test('start page: checked-in HTML is reproducible from canonical scenario recipes', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-start-page-'));
...
test('start page: offers five bounded bilingual starts without ingesting source content', () => {
  ...
  assert.match(html, /npx -y skills add tt-a1i\/flowforge --skill flowforge --agent codex --global --copy --yes/);
  assert.match(html, /npx -y skills add tt-a1i\/flowforge --skill flowforge --agent codex --copy --yes/);
  ...
  assert.match(scriptMatch[1], /flowforge:start-funnel/);
  assert.match(scriptMatch[1], /flowforge\.start\.events\.v1/);
...
test('generated artifacts omit the promotional footer and shortcut manual', () => {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-start-artifacts-'));
  try {
    for (const [type, input] of Object.entries(examples)) {
      ...
      assert.doesNotMatch(html, /Built with FlowForge/, `${type}: product signature`);
...
test('viewer gives wide screens a larger canvas without forcing a subtitle row', () => {
  ...
  assert.match(template, /max-width: var\(--flowforge-reader-width, 1440px\)/);
  assert.match(template, /FlowForge\.readerLayout = \(function \(\)/);

  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-title-hierarchy-'));
  try {
