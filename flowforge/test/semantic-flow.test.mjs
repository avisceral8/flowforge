...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-semantic-flow-'));
...
test('Semantic Flow has preset identities and motion-safe density boundaries', () => {
  ...
  assert.match(html, /@keyframes flowforge-semantic-lens-flow/);
  assert.match(html, /animation: flowforge-semantic-lens-flow 1\.35s linear 1 both/);
