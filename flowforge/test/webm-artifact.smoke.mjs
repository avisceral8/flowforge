...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-webm-artifact-'));
const externalReachSource = process.env.FLOWFORGE_REACH_CARD_SOURCE
  ? path.resolve(process.env.FLOWFORGE_REACH_CARD_SOURCE)
  : '';
const externalReachOutput = process.env.FLOWFORGE_REACH_CARD_OUTPUT
  ? path.resolve(process.env.FLOWFORGE_REACH_CARD_OUTPUT)
  : '';
assert.equal(Boolean(externalReachSource), Boolean(externalReachOutput), 'FLOWFORGE_REACH_CARD_SOURCE and FLOWFORGE_REACH_CARD_OUTPUT must be set together');
...
function executable(candidates) {
  for (const candidate of candidates) {
    ...
    try {
      return execFileSync('sh', ['-c', `command -v "$1"`, 'flowforge-which', candidate], { encoding: 'utf8' }).trim();
    } catch (_) {
...
const ffmpeg = executable([process.env.FLOWFORGE_FFMPEG, 'ffmpeg']);

assert.ok(chrome, 'Chrome/Chromium is required for the WebM artifact smoke test (or set FLOWFORGE_CHROME)');
assert.ok(ffmpeg, 'ffmpeg is required for the WebM artifact smoke test (or set FLOWFORGE_FFMPEG)');
...
try {
  ...
  async function verifyResolvedLegendContract(outputs) {
    async function inspectKinds(file, expectedKinds, theme) {
      ...
      await navigateReady(url, '!!(window.FlowForge && FlowForge.semanticLens)', `legend ${theme}`);
    ...
    await navigateReady(outputs.dataflow, '!!(window.FlowForge && FlowForge.semanticLens && FlowForge.exportMenu)', 'Dataflow database legend runtime');
    const databaseRuntime = await evaluate(cdp, sessionId, String.raw`(async function () {
      ...
      URL.createObjectURL = function (blob) {
        captured = blob.text();
        return 'blob:flowforge-dataflow-legend-smoke';
      };
      HTMLAnchorElement.prototype.click = function () {};
      try {
        await FlowForge.exportMenu.run('svg');
        var exportedText = await captured;
        var exported = new DOMParser().parseFromString(exportedText, 'image/svg+xml').documentElement;
        return {
          selected: FlowForge.semanticLens.active(),
          lensOpen: FlowForge.semanticLens.isOpen(),
    ...
    await navigateReady(outputs.custom, '!!(window.FlowForge && FlowForge.semanticLens && FlowForge.exportMenu)', 'custom legend runtime');
    const runtime = await evaluate(cdp, sessionId, String.raw`(async function () {
      ...
      var selected = FlowForge.semanticLens.active();
      var guidedActivated = FlowForge.guidedViews.activate('main', { updateUrl: false });
      var guidedActive = FlowForge.guidedViews.active();
      var visualMatrix = [];
      for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
        if (!FlowForge.preset.apply(preset)) throw new Error('could not apply preset ' + preset);
      ...
      URL.createObjectURL = function (blob) {
        captured = blob.text();
        return 'blob:flowforge-legend-smoke';
      };
      HTMLAnchorElement.prototype.click = function () {};
      try {
        await FlowForge.exportMenu.run('svg');
    ...
    await navigateReady(embedUrl, '!!(window.FlowForge && FlowForge.semanticLens)', 'embedded legend');
    ...
    await navigateReady(outputs.hidden, '!!(window.FlowForge && FlowForge.semanticLens)', 'hidden legend');
  ...
  async function verifySemanticPassportDismissal(file) {
    await navigateReady(file, '!!(window.FlowForge && FlowForge.focus && document.querySelector("#btn-focus-clear"))', 'Semantic Passport dismissal');
    const result = await evaluate(cdp, sessionId, `(() => {
      ...
      function state() {
        return { hidden: chip.hidden, active: FlowForge.focus.active() };
      }

      FlowForge.focus.set('clients', { toggle: false, updateUrl: false });
  ...
  async function verifyArchitectureDeltaNavigator(file) {
    ...
    const exportProof = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      ...
      var explorers = frames.map(function (frame) {
        ...
        return Boolean(child && child.FlowForge && child.FlowForge.focus && child.FlowForge.routeProbe && child.document.querySelector('#btn-node-finder') && child.document.querySelector('#guided-view-play'));
      });
      var svgA = FlowForge.deltaExport.canonicalSvg();
      document.querySelector('#theme').click();
      document.querySelector('#preset').click();
      document.querySelector('.change-row').click();
      var svgB = FlowForge.deltaExport.canonicalSvg();
      var parsed = new DOMParser().parseFromString(svgB, 'image/svg+xml');
      var exportStyle = parsed.querySelector('style')?.textContent || '';
      var blob = await FlowForge.deltaExport.shareCard();
  ...
  async function captureShareCard(file, label) {
    await navigateReady(file, '!!(window.FlowForge && FlowForge.exportMenu && FlowForge.exportMenu.shareCard)', label);
    const sharePayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        var blob = await FlowForge.exportMenu.shareCard();
        var bytes = new Uint8Array(await blob.arrayBuffer());
        var binary = '';
  ...
  async function captureCopiedShareCard(file, label) {
    await navigateReady(file, '!!(window.FlowForge && FlowForge.exportMenu && FlowForge.exportMenu.copyShareCard)', label);
        ...
        Object.defineProperty(navigator, 'clipboard', {
          ...
          value: {
            write: async function (items) {
              window.__flowforgeCopiedShareCard = await Promise.resolve(items[0].items['image/png']);
            }
          }
        });
        window.alert = function (message) { window.__flowforgeCopyAlert = message; };
        await FlowForge.exportMenu.copyShareCard();
        var blob = window.__flowforgeCopiedShareCard;
        if (!blob) throw new Error(window.__flowforgeCopyAlert || 'clipboard received no blob');
        var bytes = new Uint8Array(await blob.arrayBuffer());
        var binary = '';
        for (var offset = 0; offset < bytes.length; offset += 32768) {
          binary += String.fromCharCode.apply(null, bytes.subarray(offset, offset + 32768));
  ...
  async function captureRouteShareCard(file, label, sourceId, targetId, options = {}) {
    await navigateReady(file, '!!(window.FlowForge && FlowForge.routeProbe && FlowForge.exportMenu && FlowForge.exportMenu.downloadRouteShareCard)', label);
    const routePayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        window.alert = function (message) { window.__flowforgeRouteAlert = message; };
        FlowForge.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
        if (!FlowForge.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) {
          throw new Error('route did not resolve');
        }
        var snapshot = FlowForge.routeProbe.exportSnapshot();
        if (!snapshot) throw new Error('resolved route exposed no export snapshot');
        FlowForge.exportMenu.syncRouteShare();
        ...
        var duplicateGeometryRejected = FlowForge.routeProbe.exportSnapshot() === null;
        var duplicateExportError = '';
        try { await FlowForge.exportMenu.shareCard({ variant: 'route' }); }
        ...
        var emptyGeometryRejected = FlowForge.routeProbe.exportSnapshot() === null;
        ...
        try {
          blob = await FlowForge.exportMenu.downloadRouteShareCard();
          routeReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
          ...
          if (${JSON.stringify(options.journeyInvariance === true)}) {
            async function captureJourneySource(action) {
              ...
              await FlowForge.exportMenu.shareCard({ variant: 'route' });
              routeFingerprints.push(fingerprint(await captured[captureIndex]));
            }
            await captureJourneySource(function () { FlowForge.routeProbe.selectJourneyIndex(0); });
            await captureJourneySource(function () { FlowForge.routeProbe.selectJourneyIndex(Math.floor(snapshot.nodeIds.length / 2)); });
            await captureJourneySource(function () { FlowForge.routeProbe.selectJourneyIndex(snapshot.nodeIds.length - 1); });
            FlowForge.routeProbe.showOverview({ reveal: false });
            var started = FlowForge.routeProbe.playJourney();
            ...
            FlowForge.routeProbe.pauseJourney({ preserveElapsed: false });
            await captureJourneySource(function () {});
          ...
          await FlowForge.exportMenu.run('png');
          var pngReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            canonical: document.documentElement.getAttribute('data-last-export-canonical')
          ...
          var asyncClearPromise = FlowForge.exportMenu.shareCard({ variant: 'route' });
          FlowForge.routeProbe.clear({ updateUrl: false, preserveView: true });
          ...
          await FlowForge.exportMenu.downloadRouteShareCard();
          var failedReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
          ...
          return {
            ...
            staleSnapshot: FlowForge.routeProbe.exportSnapshot(),
  ...
  async function verifyDynamicReducedMotionRoute(file, label, sourceId, targetId) {
    ...
    await navigateReady(file, '!!(window.FlowForge && FlowForge.routeProbe && FlowForge.exportMenu && FlowForge.motionGovernor)', label);
    ...
    const setup = await evaluate(cdp, sessionId, `(function () {
      FlowForge.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
      if (!FlowForge.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) return { resolved: false };
      FlowForge.routeProbe.showOverview({ reveal: false });
      return {
        resolved: true,
        started: FlowForge.routeProbe.playJourney(),
        playing: FlowForge.routeProbe.isJourneyPlaying(),
  ...
  async function captureRouteVisualMatrix(file, label, sourceId, targetId) {
    await navigateReady(file, '!!(window.FlowForge && FlowForge.preset && FlowForge.routeProbe && FlowForge.exportMenu)', label);
    const matrix = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      FlowForge.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
      if (!FlowForge.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) {
        throw new Error('route did not resolve');
      }
      var identity = JSON.stringify(FlowForge.routeProbe.exportSnapshot());
      var results = [];
      for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
        if (!FlowForge.preset.apply(preset)) throw new Error('could not apply preset ' + preset);
        for (var theme of ['dark', 'light']) {
          document.documentElement.setAttribute('data-theme', theme);
          var blob = await FlowForge.exportMenu.shareCard({ variant: 'route' });
          var bytes = new Uint8Array(await blob.arrayBuffer());
          var hash = 2166136261;
          ...
          results.push({
            ...
            identity: JSON.stringify(FlowForge.routeProbe.exportSnapshot())
          });
        }
      }
  ...
  async function captureReachShareCard(file, label, originId, direction, options = {}) {
    await navigateReady(file, '!!(window.FlowForge && FlowForge.focus && FlowForge.focus.reachabilitySnapshot && FlowForge.exportMenu && FlowForge.exportMenu.downloadReachShareCard)', label);
    const reachPayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        window.alert = function (message) { window.__flowforgeReachAlert = String(message); };
        if (!FlowForge.focus.set(${JSON.stringify(originId)}, { toggle: false, updateUrl: false })) {
          throw new Error('focus origin did not resolve');
        }
        if (!FlowForge.focus.reach(${JSON.stringify(direction)}, { toggle: false, updateUrl: false, reveal: false })) {
          throw new Error('authored reach did not resolve');
        }
        var snapshot = FlowForge.focus.reachabilitySnapshot();
        if (!snapshot) throw new Error('active authored reach exposed no export snapshot');
        FlowForge.exportMenu.syncReachShare();
        ...
        svg.appendChild(duplicateCarrier);
        var duplicateGeometryRejected = FlowForge.focus.reachabilitySnapshot() === null;
        var duplicateExportError = '';
        try { await FlowForge.exportMenu.shareCard({ variant: 'reach' }); }
        catch (error) { duplicateExportError = String(error && error.message || error); }
        duplicateCarrier.remove();
        snapshot = FlowForge.focus.reachabilitySnapshot();
        ...
        try {
          var blob = await FlowForge.exportMenu.shareCard({ variant: 'reach' });
          var reachSvgText = captured[0] ? await captured[0] : '';
          var downloadedBlob = await FlowForge.exportMenu.downloadReachShareCard();
          var reachReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            width: document.documentElement.getAttribute('data-last-export-width'),
          ...
          if (${JSON.stringify(options.matrix === true)}) {
            ...
            for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
              ...
              for (var theme of ['dark', 'light']) {
                document.documentElement.setAttribute('data-theme', theme);
                var matrixBlob = await FlowForge.exportMenu.shareCard({ variant: 'reach' });
                ...
                matrix.push({
                  ...
                  identity: JSON.stringify(FlowForge.focus.reachabilitySnapshot())
                });
              }
            }
          ...
          FlowForge.focus.clearReach({ updateUrl: false });
          FlowForge.exportMenu.syncReachShare();
          ...
          await FlowForge.exportMenu.downloadReachShareCard();
          var failedReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            errorFormat: document.documentElement.getAttribute('data-last-export-error-format'),
          ...
          var canonicalIndex = captured.length;
          var canonicalBlob = await FlowForge.exportMenu.shareCard();
          var canonicalSvgText = captured[canonicalIndex] ? await captured[canonicalIndex] : '';
          var canonicalSvg = parser.parseFromString(canonicalSvgText, 'image/svg+xml').documentElement;
          ...
          return {
            ...
            hiddenAfterClear: hiddenAfterClear,
            staleSnapshot: FlowForge.focus.reachabilitySnapshot(),
            staleError: staleError,
            failedReceipt: failedReceipt,
            canonicalSize: canonicalBlob.size,
  ...
  if (externalReachSource) {
    ...
    await captureReachShareCard(
      externalReachSource,
      'external-reach',
      process.env.FLOWFORGE_REACH_CARD_ORIGIN || 'router',
      process.env.FLOWFORGE_REACH_CARD_DIRECTION || 'downstream',
  ...
  await navigateReady(output, '!!(window.FlowForge && FlowForge.motion && FlowForge.motion.canRecord())', 'motion artifact');

  const payload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
    try {
      var blob = await FlowForge.motion.recordWebm({ duration: 1400, fps: 12 });
      var bytes = new Uint8Array(await blob.arrayBuffer());
      var binary = '';
      for (var offset = 0; offset < bytes.length; offset += 32768) {
        binary += String.fromCharCode.apply(null, bytes.subarray(offset, offset + 32768));
