...
test('release prevents manifest preannouncement and smokes the exact archive before upload', () => {
  ...
  assert.match(tagGate, /require\('\.\/flowforge\/package\.json'\)\.version/);
  ...
  assert.match(build, /run: scripts\/build-zip\.sh \/tmp\/flowforge-built\.zip/);
  assert.match(smoke, /unzip -q \/tmp\/flowforge-built\.zip -d "\$package_root"/);
  assert.match(smoke, /node scripts\/package-smoke\.mjs "\$package_root\/flowforge"/);
  assert.doesNotMatch(smoke, /\bnpm\s+(?:ci|install)\b/);
  assert.match(freshness, /cmp -s \/tmp\/flowforge-built\.zip flowforge\.zip/);
  assert.match(upload, /files: flowforge\.zip/);
  assert.match(followUp, /docs\/skill-updates\/flowforge\/stable\.json/);
});
...
test('CI binds a public notifier manifest to the Release asset, tagged archive, and tag tree build', () => {
  ...
  assert.match(job, /select\(\.name == "flowforge\.zip"\)/);
  ...
  assert.match(job, /git show "v\$\{manifest_version\}:flowforge\.zip" > "\$tagged_archive"/);
...
test('package smoke rejects every dependency or repository-only artifact', () => {
  ...
  for (const { relative, kind } of forbidden) {
    const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-package-gate-'));
    try {
      fs.mkdirSync(path.join(fixture, 'bin'), { recursive: true });
      fs.writeFileSync(path.join(fixture, 'bin', 'flowforge.mjs'), '');
...
test('package smoke verifies the embedded notifier identity and local disable switch', () => {
  ...
  assert.match(source, /FLOWFORGE_UPDATE_CHECK_DISABLED: '1'/);
...
test('package smoke increments an arbitrary-precision SemVer patch without Number coercion', () => {
  const scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-package-bigint-version-'));
  const skillRoot = path.join(scratch, 'flowforge');
  try {
...
test('archive build refuses to silently omit required notifier files', () => {
  ...
  assert.match(stageSource, /flowforge\/skill-release\.json/);
  assert.match(stageSource, /flowforge\/scripts\/check-update\.mjs/);
  assert.match(stageSource, /flowforge\/scripts\/update-contract\.mjs/);
...
canonicalZipTest('package smoke rejects every dependency metadata field in a built package', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-built-package-gate-'));
  try {
    const archive = path.join(fixture, 'flowforge.zip');
    ...
    const builtPackage = path.join(extracted, 'flowforge');
...
canonicalZipTest('built archives contain the embedded notifier runtime', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-notifier-package-gate-'));
  try {
    const archive = path.join(fixture, 'flowforge.zip');
    const build = spawnSync(path.join(repoRoot, 'scripts', 'build-zip.sh'), [archive], {
      cwd: repoRoot,
    ...
    assert.ok(entries.has('flowforge/skill-release.json'));
    assert.ok(entries.has('flowforge/scripts/check-update.mjs'));
    assert.ok(entries.has('flowforge/scripts/update-contract.mjs'));
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
});
...
canonicalZipTest('archive build excludes untracked files and external symlinks from the live working tree', () => {
  ...
  const untracked = path.join(repoRoot, 'flowforge', `${marker}.txt`);
  const externalRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-package-external-'));
  const externalTarget = path.join(externalRoot, 'secret.txt');
  const externalLink = path.join(repoRoot, 'flowforge', `${marker}.link`);
  const outputRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-package-negative-'));
  const archive = path.join(outputRoot, 'flowforge.zip');
...
canonicalZipTest('archive build rejects an unmerged index and preserves an existing archive', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-package-unmerged-'));
  const scripts = path.join(fixture, 'scripts');
  const skill = path.join(fixture, 'flowforge');
  ...
  try {
    ...
    fs.writeFileSync(path.join(skill, 'package.json'), '{"name":"flowforge"}\n');
    ...
    const indexInfo = [
      `100644 ${base.stdout.trim()} 1\tflowforge/LICENSE`,
      `100644 ${ours.stdout.trim()} 2\tflowforge/LICENSE`,
      `100644 ${theirs.stdout.trim()} 3\tflowforge/LICENSE`,
      '',
    ].join('\n');
...
test('archive build rejects non-canonical Node versions before publishing output', {
  ...
  const outputRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-package-node-version-'));
  try {
    const archive = path.join(outputRoot, 'flowforge.zip');
    ...
    assert.notEqual(build.status, 0, `${build.stdout}\n${build.stderr}`);
    assert.match(build.stderr, /canonical flowforge\.zip builds require Node 22/);
...
canonicalZipTest('archive build is byte-for-byte reproducible across caller time zones without system zip', () => {
  const outputRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-package-reproducible-'));
  ...
  try {
    ...
    assert.ok(
      fs.readFileSync(utcArchive).equals(fs.readFileSync(path.join(repoRoot, 'flowforge.zip'))),
