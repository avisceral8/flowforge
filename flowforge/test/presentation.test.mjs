...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-presentation-'));
...
test('all typed renderers ship the same presentation stage contract', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /FlowForge\.presentation = \(function \(\)/, mode);
...
test('presentation keyboard behavior exits in layers and remains accessible', () => {
  ...
  assert.match(html, /FlowForge\.presentation\.toggle\(\)/);
  assert.match(html, /e\.key === 'Escape' && FlowForge\.focus\.active\(\)/);
  assert.match(html, /e\.key === 'Escape' && FlowForge\.presentation\.active\(\)/);
