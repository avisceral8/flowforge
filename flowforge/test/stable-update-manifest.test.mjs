...
test('stable release gate binds manifest tag, tree, and final archive digest', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-stable-manifest-'));
  try {
    writeJson(path.join(fixture, 'flowforge', 'package.json'), { version: '3.0.0' });
    fs.writeFileSync(path.join(fixture, 'flowforge', 'SKILL.md'), 'stable fixture\n');
    assert.equal(git(fixture, ['init']).status, 0);
    assert.equal(git(fixture, ['add', 'flowforge']).status, 0);
    assert.equal(git(fixture, [
      '-c', 'user.name=FlowForge Test',
      '-c', 'user.email=flowforge@example.invalid',
    ...
    const tree = git(fixture, ['rev-parse', 'HEAD:flowforge']);
    ...
    const manifestPath = path.join(fixture, 'docs', 'skill-updates', 'flowforge', 'stable.json');
    const manifest = {
      schemaVersion: 1,
      skillId: 'flowforge',
      channel: 'stable',
    ...
    writeJson(path.join(fixture, 'flowforge', 'package.json'), { version: '4.0.0-dev.0' });
    ...
    assert.match(treeMismatch.stderr, /treeSha .* does not match HEAD:flowforge/);
