...
const chromePath = integrationEnabled && process.env.FLOWFORGE_CHROME ? findChrome() : null;
...
function loadRuntime({
  ...
  return { language: window.FlowForgeSiteLanguage, values, url: () => new URL(currentUrl) };
}
...
test('site language runtime normalizes one entry parameter into one durable preference', () => {
  const canonical = loadRuntime({ values: new Map([['flowforge-lang', 'zh']]) });
  assert.equal(canonical.language.read(), 'zh');

  for (const legacyKey of ['flowforge-gallery-language', 'flowforge-guide-language']) {
    ...
    assert.equal(legacy.values.get('flowforge-lang'), 'zh', `${legacyKey} must migrate to the canonical key`);
  }

  const canonicalWins = loadRuntime({
    values: new Map([
      ['flowforge-lang', 'en'],
      ['flowforge-gallery-language', 'zh'],
      ['flowforge-guide-language', 'zh'],
    ]),
  });
  ...
  const secondLegacyFallback = loadRuntime({
    values: new Map([
      ['flowforge-gallery-language', 'fr'],
      ['flowforge-guide-language', 'zh'],
    ]),
  });
  assert.equal(secondLegacyFallback.language.read(), 'zh');
  assert.equal(secondLegacyFallback.values.get('flowforge-lang'), 'zh');

  const conflictingLegacy = loadRuntime({
    values: new Map([
      ['flowforge-gallery-language', 'zh'],
      ['flowforge-guide-language', 'en'],
    ]),
  });
  assert.equal(conflictingLegacy.language.read(), 'zh');
  assert.equal(conflictingLegacy.values.get('flowforge-lang'), 'zh');
  conflictingLegacy.values.set('flowforge-gallery-language', 'en');
  ...
  const explicit = loadRuntime({
    ...
    values: new Map([['flowforge-lang', 'zh']]),
  });
  assert.equal(explicit.language.read(), 'en');
  assert.equal(explicit.values.get('flowforge-lang'), 'en');
  ...
  const historyBlocked = loadRuntime({
    ...
    values: new Map([['flowforge-lang', 'en']]),
    historyError: true,
  });
  assert.equal(historyBlocked.language.read(), 'zh');
  assert.equal(historyBlocked.values.get('flowforge-lang'), 'zh');
  ...
  assert.match(source, /flowforge-gallery-language/);
  assert.match(source, /flowforge-guide-language/);
...
test('custom site builders emit every shared site asset and preserve entry, navigation, selection, and refresh state', {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-site-language-'));
  try {
    const builds = [
    ...
    assert.equal(values.get('flowforge-lang'), 'zh');
    ...
    assert.equal(values.get('flowforge-lang'), 'en');
...
test('all site pages consume one language runtime and one navigation contract', () => {
  ...
  for (const relative of pages) {
    ...
    assert.match(html, /FlowForgeSiteLanguage\.read\(/, `${relative}: shared language read missing`);
    assert.match(html, /FlowForgeSiteLanguage\.write\(/, `${relative}: shared language write missing`);
    ...
    assert.doesNotMatch(
      html,
      /localStorage\.setItem\(['"]flowforge-(?:lang|gallery-language|guide-language)['"]/,
...
test('real Chrome preserves language through entry, navigation, selection, refresh, and consistent navigation chrome', {
  skip: chromePath ? false : 'Set FLOWFORGE_CHROME to run the real site regression.',
  timeout: 60000,
}, async () => {
  ...
  try {
    ...
    await evaluate(browser, sessionId, "localStorage.setItem('flowforge-guide-language', 'zh')");
    await navigate(browser, sessionId, `${baseUrl}/index.html`);
    assert.deepEqual(await evaluate(browser, sessionId, `({
      language: document.documentElement.lang,
      stored: localStorage.getItem('flowforge-lang')
