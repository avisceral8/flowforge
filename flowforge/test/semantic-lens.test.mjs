...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-semantic-lens-'));
...
test('all typed renderers inherit one viewer-only Semantic Lens', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /FlowForge\.semanticLens = \(function \(\)/, mode);
...
test('Semantic Lens is shareable and yields cleanly to stronger reader intent', () => {
  ...
  assert.match(html, /FlowForge\.semanticLens\.clear\(\{ updateUrl: false/);
  assert.match(html, /FlowForge\.focus\.clear\(\{ updateUrl: false, preserveView: true \}\)/);
  assert.match(html, /FlowForge\.routeProbe\.clear\(\{ updateUrl: false, restoreFocus: false \}\)/);
  assert.match(html, /FlowForge\.guidedViews\.showAll\(\{ clearFocus: false, updateUrl: false \}\)/);
  assert.match(html, /if \(action === 'lens'\) return FlowForge\.semanticLens\.open\(\)/);
  assert.match(html, /e\.key === 'l' \|\| e\.key === 'L'/);
  assert.match(html, /e\.key === 'Escape' && FlowForge\.semanticLens\.isOpen\(\)/);
  assert.match(html, /e\.key === 'Escape' && FlowForge\.semanticLens\.active\(\)/);
});
