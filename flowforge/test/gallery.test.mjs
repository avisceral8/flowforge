...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-gallery-'));
...
test('generated proof gallery matches its sources, receipts, and checked-in artifacts', () => {
  ...
  assert.equal(manifest.flowforgeVersion, JSON.parse(fs.readFileSync(path.join(skillRoot, 'package.json'))).version);
  ...
  assert.match(html, /href="https:\/\/github\.com\/tt-a1i\/flowforge\/issues\/new\?template=showcase\.yml"[^>]+rel="noopener noreferrer"/);
