...
test('landing metadata describes the full technical-diagram product and trusted hero promise', () => {
  assert.match(landing, /<title>FlowForge — Technical Diagrams from Plain English<\/title>/);
  assert.match(landing, /<meta property="og:title" content="FlowForge — Technical Diagrams from Plain English">/);
  assert.match(landing, /An agent skill for Cursor, Claude Code, Codex CLI, and OpenCode/);
  assert.equal((landing.match(/npx -y skills add tt-a1i\/flowforge --skill flowforge --agent cursor --global --copy --yes/g) || []).length, 2);
