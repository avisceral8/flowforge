...
function writeValidDevelopmentFixture(root, overrides = {}) {
  ...
  const english = [
    ...
    '',
    'Raven uses manual ZIP installation: extract flowforge.zip into `~/.raven/workspace/skills`, which yields `~/.raven/workspace/skills/flowforge`; Raven is not an agent-switcher target.',
  ].join('\n');
  const chinese = [
    ...
    `当前开发版本：\`v${version}\``,
    '',
    'Raven 使用 ZIP 手动安装：将 flowforge.zip 解压到 `~/.raven/workspace/skills`，解压后会得到 `~/.raven/workspace/skills/flowforge`；Raven 不属于 Agent 切换器目标。',
  ].join('\n');
  const files = {
    'flowforge/package.json': JSON.stringify({ version }),
    'flowforge/package-lock.json': JSON.stringify({ version, packages: { '': { version } } }),
    'flowforge/skill-release.json': JSON.stringify({
      schemaVersion: 1,
      skillId: 'flowforge',
      channel: 'development',
      version,
    ...
    'docs/skill-updates/flowforge/stable.json': stableUpdateManifest('2.12.0'),
    'flowforge/SKILL.md': '---\nmetadata:\n  version: "2.13"\n---\n',
    'flowforge/assets/template.html': '<meta name="generator" content="flowforge 2.13.0-dev.0">',
    'CHANGELOG.md': [
      '# Changelog',
      '',
      '## [Unreleased]',
    ...
    'scripts/start-template.html': 'development · 开发版 · [[FLOWFORGE_VERSION]]',
    'scripts/guide-template.html': 'development · 开发版 · [[FLOWFORGE_VERSION]]',
    'scripts/gallery-template.html': 'development · 开发版 · [[FLOWFORGE_VERSION]]',
    'docs/index.html': `<span>development · v${version} · 开发版 · 9/9 checks</span><p>Raven manual ZIP / ZIP 手动安装: extract flowforge.zip into ~/.raven/workspace/skills, which yields ~/.raven/workspace/skills/flowforge; 将 flowforge.zip 解压到 ~/.raven/workspace/skills，解压后会得到 ~/.raven/workspace/skills/flowforge; not an agent-switcher target.</p>`,
    'docs/start.html': `<span>development · v${version} · 开发版</span><p>Raven manual ZIP / ZIP 手动安装: extract flowforge.zip into ~/.raven/workspace/skills, which yields ~/.raven/workspace/skills/flowforge; 将 flowforge.zip 解压到 ~/.raven/workspace/skills，解压后会得到 ~/.raven/workspace/skills/flowforge; not an agent-switcher target.</p>`,
...
function writeValidStableFixture(root, overrides = {}) {
  ...
  const files = {
    ...
    'docs/skill-updates/flowforge/stable.json': stableUpdateManifest(version),
    'flowforge/SKILL.md': '---\nmetadata:\n  version: "2.13"\n---\n',
    'flowforge/assets/template.html': '<meta name="generator" content="flowforge 2.13.0">',
    'CHANGELOG.md': [
      '# Changelog',
      '',
      '## [Unreleased]',
    ...
    'README_EN.md': english,
    'README_ZH.md': chinese,
    'scripts/start-template.html': 'stable · 稳定版 · [[FLOWFORGE_VERSION]]',
    'scripts/guide-template.html': 'stable · 稳定版 · [[FLOWFORGE_VERSION]]',
    'scripts/gallery-template.html': 'stable · 稳定版 · [[FLOWFORGE_VERSION]]',
    'docs/index.html': `<span>stable · v${version} · 稳定版 · 9/9 checks</span><p>Raven manual ZIP / ZIP 手动安装: extract flowforge.zip into ~/.raven/workspace/skills, which yields ~/.raven/workspace/skills/flowforge; 将 flowforge.zip 解压到 ~/.raven/workspace/skills，解压后会得到 ~/.raven/workspace/skills/flowforge; not an agent-switcher target.</p>`,
    'docs/start.html': `<span>stable · v${version} · 稳定版</span><p>Raven manual ZIP / ZIP 手动安装: extract flowforge.zip into ~/.raven/workspace/skills, which yields ~/.raven/workspace/skills/flowforge; 将 flowforge.zip 解压到 ~/.raven/workspace/skills，解压后会得到 ~/.raven/workspace/skills/flowforge; not an agent-switcher target.</p>`,
...
test('an empty Unreleased section accepts a coherent stable release identity', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-release-identity-'));
  try {
...
test('stable release preparation allows only the immediate prior public manifest', () => {
  ...
  try {
    ...
    writeFile(fixture, 'docs/skill-updates/flowforge/stable.json', stableUpdateManifest('2.11.0'));
  ...
  try {
    writeValidDevelopmentFixture(fixture, {
      'flowforge/skill-release.json': JSON.stringify({
        ...
        channel: 'stable',
        version: '2.12.0',
        source: { repository: 'https://example.com/untrusted/flowforge' },
...
test('the published update manifest must track the newest stable changelog release', () => {
  ...
  try {
    writeValidDevelopmentFixture(fixture, {
      'docs/skill-updates/flowforge/stable.json': stableUpdateManifest('2.11.0'),
    });
...
test('the published update manifest must use a canonical UTC timestamp', () => {
  ...
  try {
    ...
    writeValidDevelopmentFixture(fixture, {
      'docs/skill-updates/flowforge/stable.json': JSON.stringify(manifest),
    });
...
test('real Unreleased changes cannot reuse a stable published package identity', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-release-identity-'));
  try {
    writeFile(fixture, 'flowforge/package.json', JSON.stringify({ version: '2.12.0' }));
...
test('package, lockfile, Skill metadata, escaped Shields badge, and public docs share one development identity', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-release-identity-'));
  try {
    writeFile(fixture, 'flowforge/package.json', JSON.stringify({ version: '2.13.0-dev.0' }));
    writeFile(fixture, 'flowforge/package-lock.json', JSON.stringify({
      version: '2.12.0',
      packages: { '': { version: '2.12.0' } },
    }));
    writeFile(fixture, 'flowforge/SKILL.md', '---\nmetadata:\n  version: "2.12"\n---\n');
    writeFile(fixture, 'CHANGELOG.md', [
      '# Changelog',
      '',
      '## [Unreleased]',
...
test('landing proof receipt matches the current nine-check artifact contract', () => {
  ...
  try {
    writeValidDevelopmentFixture(fixture, {
      'docs/index.html': '<span>development · v2.13.0-dev.0 · 开发版 · 8/8 checks</span><p>Raven manual ZIP / ZIP 手动安装: extract flowforge.zip into ~/.raven/workspace/skills, which yields ~/.raven/workspace/skills/flowforge; 将 flowforge.zip 解压到 ~/.raven/workspace/skills，解压后会得到 ~/.raven/workspace/skills/flowforge; not an agent-switcher target.</p>',
    });
  ...
  try {
    writeValidDevelopmentFixture(fixture, {
      'docs/index.html': [
        ...
        '<p>Raven manual ZIP / ZIP 手动安装: extract flowforge.zip into ~/.raven/workspace/skills, which yields ~/.raven/workspace/skills/flowforge; 将 flowforge.zip 解压到 ~/.raven/workspace/skills，解压后会得到 ~/.raven/workspace/skills/flowforge; not an agent-switcher target.</p>',
      ].join('\n'),
    });
...
test('Raven instructions reject extracting the archive into the final Skill directory', () => {
  ...
  try {
    const nestedEnglish = [
      ...
      '',
      'Raven is manual ZIP only: extract flowforge.zip into `~/.raven/workspace/skills/flowforge`; Raven is not an agent-switcher target.',
    ].join('\n');
    writeValidDevelopmentFixture(fixture, {
    ...
    const result = runCheck(fixture);
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /extract flowforge\.zip into ~\/\.raven\/workspace\/skills, yielding ~\/\.raven\/workspace\/skills\/flowforge/);
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
});
...
test('renderer template generator carries the complete package prerelease identity', () => {
  ...
  try {
    writeValidDevelopmentFixture(fixture, {
      'flowforge/assets/template.html': '<meta name="generator" content="flowforge 2.12.0">',
    });
...
test('generated public-page templates keep a development marker and version placeholder', () => {
  ...
  try {
    ...
    const result = runCheck(fixture);
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /scripts\/gallery-template\.html must use \[\[FLOWFORGE_VERSION\]\] with development and 开发版 labels/);
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
});
