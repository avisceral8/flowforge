...
function fixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-evidence-repo-'));
  ...
  git(root, 'init');
  git(root, 'config', 'user.name', 'FlowForge Tests');
  git(root, 'config', 'user.email', 'flowforge@example.test');
...
test('repository evidence is revision-verified, receipt-backed, searchable, and export-clean', () => {
  ...
  assert.match(html, /FlowForge\.sourceEvidence = \(function \(\)/);
  ...
  assert.match(html, /FlowForge\.sourceEvidence\.installBeacons\(\)/);
...
test('repository evidence is opt-in and never appears in ordinary artifacts', () => {
  const output = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-no-evidence-')), 'plain.html');
  const input = path.join(skillRoot, 'examples', 'web-app.architecture.json');
  const result = run(['render', 'architecture', input, output]);
  assert.equal(result.status, 0, result.stderr);
  const html = fs.readFileSync(output, 'utf8');
  assert.doesNotMatch(html, /id="flowforge-source-evidence-data"/);
