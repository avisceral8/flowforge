...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-animation-'));
...
test('signal-flow preset reaches the page, SVG, and motion export surface', () => {
  ...
  assert.match(html, /FlowForge\.motion = \{ canRecord: canRecordMotion, recordWebm: recordWebm \}/);
  assert.match(html, /recorder\.requestData\(\)/);
  assert.match(html, /aria-label="Diagram view controls"/);
  assert.match(html, /FlowForge\.focus = \(function \(\)/);
  assert.match(html, /FlowForge\.view = \(function \(\)/);
...
test('editorial preset reaches every visual surface and all five typed renderers', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /data-preset-badge-editorial-plate="FLOWFORGE \/ PLATE 04"/, mode);
...
for (const [mode, example] of Object.entries(CASES)) {
  test(`${mode}: trace animation annotates svg, edges, and nodes`, () => {
    ...
    assert.match(svg, /aria-labelledby="flowforge-diagram-title flowforge-diagram-description"/);
    assert.match(svg, /<title id="flowforge-diagram-title">[^<]+<\/title>/);
    assert.match(svg, /<desc id="flowforge-diagram-description">[^<]+<\/desc>/);
