...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-route-share-card-'));
...
test('Route Share Card menu lifecycle excludes hidden state from keyboard navigation', () => {
  ...
  assert.match(html, /function clear\(options\)[\s\S]*?FlowForge\.exportMenu\.syncRouteShare\(\)/);
  assert.match(html, /function showResult\(result, options\)[\s\S]*?FlowForge\.exportMenu\.syncRouteShare\(\)/);
...
test('Route Share Card reuses one 1200x630 variant seam and publishes a truthful receipt', () => {
  ...
  assert.match(html, /var snapshot = FlowForge\.routeProbe && FlowForge\.routeProbe\.exportSnapshot\(\)/);
  ...
  assert.match(html, /clearExportReceipt\(\);[\s\S]*?var snapshot = FlowForge\.routeProbe/);
...
test('skill and READMEs describe the optional Export variant and show one real card without changing the hero', () => {
  ...
  for (const readme of ['README.md', 'README_EN.md', 'README_ZH.md']) {
    ...
    assert.match(text, /docs\/assets\/flowforge-route-share-card\.png/, readme);
  }

  const png = fs.readFileSync(path.join(repoRoot, 'docs/assets/flowforge-route-share-card.png'));
