...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-delta-'));
...
test('compare CLI writes a deterministic three-state artifact and complete sidecar receipt', () => {
  ...
  assert.match(firstHtml, /window\.FlowForge\.deltaExport = \{ canonicalSvg: canonicalDeltaSvg, shareCard/);
  ...
  assert.match(firstHtml, /document\.querySelectorAll\('#flowforge-compare-receipt'\)\.length !== 1/);
...
test('artifact validation fails closed on missing, duplicate, or self-blessed review identity', () => {
  ...
  const receiptNode = html.match(/<script id="flowforge-compare-receipt"[\s\S]*?<\/script>/)?.[0];
