...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-cli-'));
...
fs.writeFileSync(process.env.FLOWFORGE_TEST_OPEN_LOG, JSON.stringify({
...
test('cli: help lists commands and diagram types', () => {
  ...
  assert.match(result.stdout, /flowforge guide \[scenario or question\]/);
  assert.match(result.stdout, /flowforge doctor/);
  assert.match(result.stdout, /flowforge demo \[output-directory\]/);
...
test('cli: doctor reports a complete installation is ready', () => {
  ...
  assert.match(result.stdout, /FlowForge is ready\./);
});
...
test('cli: doctor identifies an incomplete installation', () => {
  ...
  fs.copyFileSync(cli, path.join(incompleteBin, 'flowforge.mjs'));

  const result = spawnSync(process.execPath, [path.join(incompleteBin, 'flowforge.mjs'), 'doctor'], {
    cwd: incompleteRoot,
    encoding: 'utf8',
  });
  ...
  assert.match(result.stderr, /FlowForge is not ready: \d+ required files? missing\./);
});
...
test('cli: doctor rejects a corrupt standalone validator', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(corruptRoot, 'bin/flowforge.mjs'), 'doctor'], {
    cwd: corruptRoot,
    encoding: 'utf8',
  });
  ...
  assert.match(result.stderr, /FlowForge is not ready: 1 runtime check failed\./);
});
...
test('cli: examples renders from an installed skill', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(installedRoot, 'bin/flowforge.mjs'), 'examples'], {
    cwd: installedRoot,
    encoding: 'utf8',
  });
...
test('cli: guide lists all scenario recipes by diagram type', () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /FlowForge scenario recipes \(11\)/);
...
test('cli: guide works from an installed skill without node_modules', () => {
  ...
  copyInstalledSkill(installedRoot);
  const installedCli = path.join(installedRoot, 'bin/flowforge.mjs');
...
test('cli: demo creates a ready-to-open diagram in a chosen directory', () => {
  ...
  assert.match(result.stdout, /flowforge render architecture/);
});
...
test('cli: demo defaults to the current directory', () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.equal(fs.existsSync(path.join(workingDirectory, 'flowforge-demo.html')), true);
});
...
test('cli: visual-check returns a skipped receipt with exit 2 when Chrome is unavailable', () => {
  ...
  const result = run(['visual-check', out, '--json'], {
    env: { ...process.env, FLOWFORGE_CHROME: missingChrome },
  });
...
test('cli: preview runs from an installed skill without node_modules and exits cleanly', { timeout: 30000 }, async () => {
  ...
  assert.deepEqual(fs.readdirSync(path.dirname(output)).filter((name) => name.startsWith('.flowforge-preview-')), []);
});
...
test('cli: deliver preserves the previous artifact when the final check fails', () => {
  ...
  assert.deepEqual(
    fs.readdirSync(path.dirname(out)).filter((name) => name.includes('.flowforge-delivery-')),
    [],
  );
});
