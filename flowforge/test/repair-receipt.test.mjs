...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-repair-receipt-'));
...
test('repair receipt: validate and deliver share exact Clean Flow evidence while delivery preserves the trusted artifact', () => {
  ...
  assert.deepEqual(fs.readdirSync(tmp).filter((name) => name.startsWith('.flowforge-delivery-')), []);
});
