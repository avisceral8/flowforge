...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-route-probe-'));
...
test('all typed renderers inherit one viewer-only Route Probe', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /FlowForge\.routeProbe = \(function \(\)/, mode);
...
test('Route Probe turns a two-node question into a readable route receipt and stable link', () => {
  ...
  assert.match(html, /FlowForge\.view\.reveal\(result\.nodes, \{ includeNeighbors: false, reason: 'route' \}\)/);
...
test('Route Probe hands large-diagram endpoint selection to a reachability-aware Finder', () => {
  ...
  assert.match(html, /FlowForge\.finder\.open\(\{ context: context \}\)/);
...
test('Route Probe keeps pointer, keyboard, motion, embed, and export boundaries explicit', () => {
  ...
  assert.match(html, /e\.key === 'Escape' && FlowForge\.routeProbe\.active\(\)/);
  ...
  assert.match(html, /@keyframes flowforge-route-probe-flow/);
