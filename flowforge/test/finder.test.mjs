...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-finder-'));
...
test('all typed renderers ship the same geometry-neutral node finder', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /FlowForge\.finder = \(function \(\)/, mode);
    assert.match(html, /svg\.querySelectorAll\('\[data-node-id\]'\)/, mode);
    assert.doesNotMatch(svg(html), /node-finder|FlowForge\.finder|Find a node/, mode);
  }
});
...
test('finder searches semantic ids and labels, then delegates to focus and reveal', () => {
  ...
  assert.match(html, /FlowForge\.guidedViews\.showAll\(\{ clearFocus: false, updateUrl: false \}\)/);
  assert.match(html, /FlowForge\.view\.reset\(\{ automatic: true \}\)/);
  assert.match(html, /FlowForge\.focus\.set\(id, \{ toggle: false \}\)/);
  assert.match(html, /FlowForge\.view\.reveal\(\[id\], \{ includeNeighbors: true, reason: 'finder' \}\)/);
...
test('finder becomes a contextual Route Probe endpoint picker without changing semantic focus', () => {
  ...
  assert.match(html, /FlowForge\.routeProbe\.finderContext\(\)/);
  ...
  assert.match(html, /FlowForge\.routeProbe\.choose\(id\)/);
...
test('finder is keyboard accessible, mobile-pinned, and subordinate to embed mode', () => {
  ...
  assert.match(html, /FlowForge\.finder\.open\(\)/);
  assert.match(html, /event\.key === 'ArrowDown'/);
  assert.match(html, /event\.key === 'ArrowUp'/);
  assert.match(html, /event\.key === 'Escape'/);
  assert.match(html, /event\.stopPropagation\(\)/);
  assert.match(html, /FlowForge\.exportMenu\.isOpen\(\)\) FlowForge\.exportMenu\.close\(false\)/);
