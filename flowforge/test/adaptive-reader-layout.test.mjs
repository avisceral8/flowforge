...
const reader = template.slice(
  ...
  template.indexOf('FlowForge.view = (function ()'),
);
...
test('wide desktop diagrams use one height-budgeted reader shell instead of breakpoint jumps', () => {
  ...
  assert.match(reader, /html\.style\.setProperty\('--flowforge-reader-width', rounded \+ 'px'\)/);
});
...
test('desktop readability budget matches the minimum adaptive reader at 1440 by 900', () => {
  ...
  assert.match(template, /@media \(min-width: 768px\) and \(max-height: 1100px\)[\s\S]*?\.diagram-container \{[\s\S]*?padding: 0\.875rem;[\s\S]*?padding-bottom: calc\(0\.875rem \+ var\(--flowforge-nav-reserve\)\);/);
