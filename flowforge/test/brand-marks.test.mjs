...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-brand-marks-'));
const cases = {
...
test('capture command returns a digest-pinned brand object that renders reproducibly', async () => {
  ...
  try {
    ...
    const capture = await runCliAsync(['brands', 'capture', url, '--json'], { FLOWFORGE_BRAND_ALLOW_PRIVATE: '1' });
    ...
    const rendered = await renderAsync('architecture', input, 'captured-link', { FLOWFORGE_BRAND_ALLOW_PRIVATE: '1' });
...
test('a pinned brand fails closed when the remote icon digest changes', async () => {
  ...
  try {
    ...
    const result = await runCliAsync(['validate', 'architecture', input, '--json'], { FLOWFORGE_BRAND_ALLOW_PRIVATE: '1' });
    assert.equal(result.status, 1, result.stderr || result.stdout);
    const receipt = JSON.parse(result.stdout);
...
test('a pinned brand keeps identical artifact metadata when the remote page title changes', async () => {
  ...
  try {
    ...
    const first = await renderAsync('architecture', input, 'stable-title-first', { FLOWFORGE_BRAND_ALLOW_PRIVATE: '1' });
    const second = await renderAsync('architecture', input, 'stable-title-second', { FLOWFORGE_BRAND_ALLOW_PRIVATE: '1' });
...
test('each prepare call rechecks pinned remote bytes instead of trusting a process-wide cache', async () => {
  ...
  await new Promise((resolve) => server.listen(0, '127.0.0.1', resolve));
  const priorAllowPrivate = process.env.FLOWFORGE_BRAND_ALLOW_PRIVATE;
  process.env.FLOWFORGE_BRAND_ALLOW_PRIVATE = '1';
  try {
    const address = server.address();
    const diagram = {
      components: [{
    ...
    if (priorAllowPrivate === undefined) delete process.env.FLOWFORGE_BRAND_ALLOW_PRIVATE;
    else process.env.FLOWFORGE_BRAND_ALLOW_PRIVATE = priorAllowPrivate;
    await new Promise((resolve) => server.close(resolve));
  }
});
...
test('rendering many pinned brands limits concurrent remote capture work', async () => {
  ...
  try {
    ...
    const rendered = await renderAsync('architecture', input, 'bounded-capture', { FLOWFORGE_BRAND_ALLOW_PRIVATE: '1' });
    assert.equal(rendered.status, 0, rendered.stderr || rendered.stdout);
...
test('rendering many pinned brands shares one diagram capture deadline', async () => {
  ...
  try {
    ...
    const rendered = await renderAsync('architecture', input, 'diagram-deadline', {
      FLOWFORGE_BRAND_ALLOW_PRIVATE: '1',
      FLOWFORGE_BRAND_CAPTURE_TIMEOUT_MS: '100',
    });
...
test('capture applies one total deadline across the page and icon requests', async () => {
  ...
  try {
    ...
    const capture = await runCliAsync(
      ...
      {
        FLOWFORGE_BRAND_ALLOW_PRIVATE: '1',
        FLOWFORGE_BRAND_CAPTURE_TIMEOUT_MS: '150',
      },
    );
    assert.notEqual(capture.status, 0, capture.stdout);
...
test('unknown preset names fail with a repairable public CLI diagnostic', () => {
  ...
  assert.ok(receipt.diagnostics.some((entry) => entry.supportedFixes.some((fix) => fix.includes('flowforge brands'))));
});
