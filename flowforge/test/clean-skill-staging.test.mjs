...
function repositoryFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-clean-stage-'));
  write(root, 'flowforge/package.json', JSON.stringify({
    name: 'flowforge-fixture',
    scripts: { test: 'node --test' },
    devDependencies: { ajv: '1.0.0' },
  }));
  write(root, 'flowforge/package-lock.json', '{}\n');
  write(root, 'flowforge/skill-release.json', '{}\n');
  write(root, 'flowforge/scripts/check-update.mjs', 'export {};\n');
  write(root, 'flowforge/scripts/update-contract.mjs', 'export {};\n');
  write(root, 'flowforge/renderers/shared/generated-validators.mjs', 'export {};\n');
  write(root, 'flowforge/test/repository-only.test.mjs', 'throw new Error();\n');
  git(root, ['init']);
  return root;
}
...
test('clean staging preserves index modes and strips repository-only package metadata', () => {
  ...
  try {
    write(root, 'flowforge/bin/executable.mjs', '#!/usr/bin/env node\n', 0o755);
    write(root, 'flowforge/runtime/test/required.dat', 'runtime fixture\n');
    git(root, ['add', 'flowforge']);
...
test('clean staging rejects a symlink in a tracked file ancestor before copying bytes', (t) => {
  ...
  const destination = path.join(root, 'staged-skill');
  try {
    const runtime = path.join(root, 'flowforge', 'runtime');
    write(root, 'flowforge/runtime/payload.txt', 'tracked fixture\n');
    git(root, ['add', 'flowforge']);
    ...
    assert.throws(
      () => stageCleanSkill({ repoRoot: root, destination }),
      /refusing to package path through symlink: flowforge\/runtime/,
    );
...
test('clean staging rejects tracked symlinks before reading through them', (t) => {
  ...
  try {
    ...
    const linked = path.join(root, 'flowforge', 'linked.txt');
    try {
    ...
    assert.throws(
      () => stageCleanSkill({ repoRoot: root, destination }),
      /refusing to package tracked symlink: flowforge\/linked\.txt/,
    );
    assert.equal(fs.existsSync(destination), false);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
...
test('clean staging snapshots unstaged tracked bytes before a source ancestor can be swapped', (t) => {
  ...
  let swapped = false;
  try {
    const payload = write(root, 'flowforge/runtime/payload.txt', 'indexed fixture\n');
    write(root, 'outside-runtime/payload.txt', 'external secret\n');
    git(root, ['add', 'flowforge']);
...
test('clean staging rejects a source ancestor swapped during preflight traversal', (t) => {
  ...
  try {
    ...
    write(root, 'outside-runtime/payload.txt', 'external secret\n');
    git(root, ['add', 'flowforge']);
    const canonicalRuntime = path.join(fs.realpathSync(root), 'flowforge', 'runtime');
    ...
    assert.throws(
      () => stageCleanSkill({ repoRoot: root, destination }),
      /(?:tracked package path changed before it could be read: flowforge\/|tracked package input is missing or unreadable: flowforge\/runtime\/payload\.txt)/,
    );
...
test('clean staging reports the Git spawn error when Git cannot start', () => {
  ...
  try {
    ...
    assert.match(result.stderr, /unable to enumerate tracked FlowForge files: .*ENOENT/);
    assert.doesNotMatch(result.stderr, /tracked FlowForge paths must be valid UTF-8/);
    assert.equal(fs.existsSync(destination), false);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
