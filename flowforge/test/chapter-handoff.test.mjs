...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flowforge-chapter-handoff-'));
...
test('handoff holds one truthful anchor then settles through one finite camera transaction', () => {
  ...
  assert.match(html, /FlowForge\.motionGovernor\.claim\('handoff'/);
  assert.match(html, /FlowForge\.motionGovernor\.release\(handoff\.ownerToken\)/);
...
test('latest intent, manual takeover, Still, reduced motion, and hidden pages cleanly settle', () => {
  ...
  assert.match(html, /FlowForge\.guidedViews\.cancelHandoff\(reason \|\| 'manual'\)/);
