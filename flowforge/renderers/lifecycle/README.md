...
Render `diagram_type: "lifecycle"` JSON files into the standard FlowForge HTML
template.

```bash
node flowforge/renderers/lifecycle/render-lifecycle.mjs input.lifecycle.json output.html
```

The renderer validates input against `flowforge/schemas/lifecycle.schema.json`
...
`flowforge/examples/agent-run.lifecycle.json`.

The schema lives at:

```text
flowforge/schemas/lifecycle.schema.json
```
