...
const SVG_SLOT_RE = /      <!-- FLOWFORGE:SVG_SLOT_START -->[\s\S]*?      <!-- FLOWFORGE:SVG_SLOT_END -->/;
const CARDS_SLOT_RE = /    <!-- FLOWFORGE:CARDS_SLOT_START -->[\s\S]*?    <!-- FLOWFORGE:CARDS_SLOT_END -->/;
const SUBTITLE_SLOT_RE = /^([ \t]*)<p class="subtitle">\[Subtitle description\]<\/p>[ \t]*(\r?\n)?/m;
const GUIDED_VIEWS_PLACEHOLDER = '<!-- FLOWFORGE:GUIDED_VIEWS_DATA -->';
const SOURCE_EVIDENCE_PLACEHOLDER = '    <!-- FLOWFORGE:SOURCE_EVIDENCE_DATA -->';
const I18N_PLACEHOLDER = '    <!-- FLOWFORGE:I18N_DATA -->';
...
export function applyTemplate(template, {
  ...
  if (!SVG_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: template missing FLOWFORGE:SVG_SLOT sentinel');
  }
  if (!CARDS_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: template missing FLOWFORGE:CARDS_SLOT sentinel');
  }
  ...
  const renderedSubtitle = typeof subtitle === 'string' && subtitle.trim()
    ...
    : '';
  const i18nData = `    <script id="flowforge-i18n-data" type="application/json">${i18nJson}</script>`;
  ...
  return templateWithI18n
    ...
    .replace(GUIDED_VIEWS_PLACEHOLDER, () => `<script id="flowforge-guided-views-data" type="application/json">${guidedViewsJson}</script>`)
    .replace(SOURCE_EVIDENCE_PLACEHOLDER, () => sourceEvidence
      ? `    <script id="flowforge-source-evidence-data" type="application/json">${sourceEvidenceJson}</script>`
      : '');
}
