...
const canvasType = process.env.FLOWFORGE_CANVAS_TYPE || 'journey';
const { diagram: workflow, template, outPath } = await loadDiagramWithBrandMarks({
  rendererDir: __dirname,
  diagramType: canvasType,
  defaultExample: `order-to-cash.${canvasType}.json`
});
...
function renderNode(node) {
  ...
  const sub = hasSub
    ? `\n          <text data-detail="context" x="${node.cx}" y="${node.y + 41}" class="t-muted" font-size="${sublabelFontSize}" text-anchor="middle">${esc(node.sublabel)}</text>`
    : '';
  const tag = node.tag
