...
Render `diagram_type: "dataflow"` JSON files into the standard FlowForge HTML
template.

```bash
node flowforge/renderers/dataflow/render-dataflow.mjs input.dataflow.json output.html
```

The renderer validates input against `flowforge/schemas/dataflow.schema.json`
...
`flowforge/examples/product-analytics.dataflow.json`.

The schema lives at:

```text
flowforge/schemas/dataflow.schema.json
```
...
visual-only because FlowForge has no compiled edge-kind facts in this slice. A
