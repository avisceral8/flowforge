...
explicit authored viewBox without overlap, FlowForge omits the complete legend
...
by `flowforge brands --json`, or a digest-pinned `{ "url", "sha256" }` object
returned by `flowforge brands capture <url> --json`. Known IDs and known-brand
...
strictly FlowForge judges composition. `engineering_profile` is an optional
