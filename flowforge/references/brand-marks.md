...
1. Search the built-in catalogue when the request names a recognizable brand:
   ...
   ```bash
   node bin/flowforge.mjs brands "Claude" --json
   ```
...
3. If there is no catalogue match and the user supplied the official website,
   ...
   ```bash
   node bin/flowforge.mjs brands capture "https://partner.example.com" --json
   ```
...
Use `node bin/flowforge.mjs brands --json` to inspect all canonical IDs, aliases,
